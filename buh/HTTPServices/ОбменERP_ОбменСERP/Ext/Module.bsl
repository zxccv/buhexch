...
Функция ПолучитьИменаСвойствСоЗначениямиДата()
	...
	МассивДаты.Добавить("ПланируемаяДатаВыплаты");	
	//МассивДаты.Добавить("");	
	//МассивДаты.Добавить("");	
	//МассивДаты.Добавить("");	
	//МассивДаты.Добавить("");	
...
Функция НайтиСоздатьОбъектДокумента(УИДДокумента, ТипДокумента)
	...
	Иначе
		...
		Если ДокументСсылка <> Неопределено И НЕ ОбщегоНазначения.СсылкаСуществует(ДокументСсылка) Тогда
...
Процедура ЗагрузитьПроводкиДокумента(ДокументОбъект, структураПроводки, ЗапрашиваемыеОбъекты)
	...
	Для Каждого структураПроводка Из структураПроводки Цикл
		...
		Если ЗначениеЗаполнено(структураПроводка.СубконтоДт1) Тогда
			...
			Иначе
				Если структураПроводка.ВидСубконтоДт1 = "Справочник.Организации" 
					ИЛИ структураПроводка.ВидСубконтоДт1 = "Справочник.Контрагенты" Тогда
					ЗначениеСубконто = НайтиОбъектПоИдентификаторуНов(структураПроводка.СубконтоДт1,Тип("СправочникСсылка.Контрагенты"));
				ИначеЕсли структураПроводка.ВидСубконтоДт1 = "Справочник.ОбъектыЭксплуатации" Тогда
					Если СчетДт = ПланыСчетов.Хозрасчетный.СтроительствоОбъектовОсновныхСредств Тогда  
						ЗначениеСубконто = НайтиОбъектПоИдентификаторуНов(структураПроводка.СубконтоДт1, Тип("СправочникСсылка.ОбъектыСтроительства"));
						ТипПриемника = "ОбъектСтроительства";
					Иначе
						ЗначениеСубконто = НайтиОбъектПоИдентификаторуНов(структураПроводка.СубконтоДт1, Тип("СправочникСсылка.ОсновныеСредства")); 
						ТипПриемника = "ОсновноеСредство";
					КонецЕсли;
				Иначе
				...
				Иначе
					СтруктураЗапрашиваемогоОбъекта = Новый Структура("УИДВладельца, УИДОбъекта, КлючВСтруктуре, ТипПриемника", "", структураПроводка.СубконтоДт1, "", ТипПриемника);
					ЗапрашиваемыеОбъекты.Добавить(СтруктураЗапрашиваемогоОбъекта);					
				КонецЕсли;
			КонецЕсли;
		КонецЕсли;    
		...
		Если ЗначениеЗаполнено(структураПроводка.СубконтоКт1) Тогда
			...
			Иначе         
				Если структураПроводка.ВидСубконтоКт1 = "Справочник.Организации" 
					ИЛИ структураПроводка.ВидСубконтоКт1 = "Справочник.Контрагенты" Тогда
					ЗначениеСубконто = НайтиОбъектПоИдентификаторуНов(структураПроводка.СубконтоКт1,Тип("СправочникСсылка.Контрагенты"));
				ИначеЕсли структураПроводка.ВидСубконтоКт1 = "Справочник.ОбъектыЭксплуатации" Тогда
					Если СчетКт = ПланыСчетов.Хозрасчетный.СтроительствоОбъектовОсновныхСредств Тогда  
						ЗначениеСубконто = НайтиОбъектПоИдентификаторуНов(структураПроводка.СубконтоКт1, Тип("СправочникСсылка.ОбъектыСтроительства")); 
						ТипПриемника = "ОбъектСтроительства";
					Иначе
						ЗначениеСубконто = НайтиОбъектПоИдентификаторуНов(структураПроводка.СубконтоКт1, Тип("СправочникСсылка.ОсновныеСредства")); 
						ТипПриемника = "ОсновноеСредство";
					КонецЕсли;
				Иначе
				...
				Иначе
					СтруктураЗапрашиваемогоОбъекта = Новый Структура("УИДВладельца, УИДОбъекта, КлючВСтруктуре, ТипПриемника", "", структураПроводка.СубконтоКт1, "", ТипПриемника);
					ЗапрашиваемыеОбъекты.Добавить(СтруктураЗапрашиваемогоОбъекта);					
				КонецЕсли;
			КонецЕсли;
		КонецЕсли;
...
Процедура ЗагрузитьОрганизацию(структураОбъект, ОписаниеОшибок, ЗапрашиваемыеОбъекты)
	...
	Если УИДЗаполнен(структураОбъект.УчетнаяЗаписьОбмена)Тогда
		...
		Если Не ЗначениеЗаполнено(ОрганизацияОбъект.УчетнаяЗаписьОбмена) Тогда  
			...
			СтруктураЗапрашиваемогоОбъекта = Новый Структура("УИДВладельца, УИДОбъекта, КлючВСтруктуре, ТипПриемника", структураОбъект.УИД, структураОбъект.УчетнаяЗаписьОбмена, "УчетнаяЗаписьОбмена", XmlТипЗнч(Справочники.УчетныеЗаписиДокументооборота.ПустаяСсылка()).ИмяТипа);
			ЗапрашиваемыеОбъекты.Добавить(СтруктураЗапрашиваемогоОбъекта);					
		КонецЕсли;                                                            
	Иначе
...
Процедура ЗагрузитьСтатьюЗатрат(структураОбъект, ОписаниеОшибок, ЗапрашиваемыеОбъекты)
	...
	Если ИскомаяСсылка = Неопределено Тогда  
		...
		Если структураОбъект.Наименование = "Амортизационная премия" Тогда
			ТекущийОбъект = Справочники[ТипСправочника].АмортизационнаяПремия.ПолучитьОбъект();
		ИначеЕсли структураОбъект.Наименование = "Командировочные расходы" Тогда
			ТекущийОбъект = Справочники[ТипСправочника].КомандировочныеРасходы.ПолучитьОбъект();
		ИначеЕсли структураОбъект.Наименование = "Малоценное оборудование и запасы" Тогда
			ТекущийОбъект = Справочники[ТипСправочника].ПриобретениеМалоценногоОборудованияИЗапасов.ПолучитьОбъект();
		ИначеЕсли структураОбъект.Наименование = "Неамортизируемое имущество" Тогда
			ТекущийОбъект = Справочники[ТипСправочника].НеамортизируемоеИмущество.ПолучитьОбъект();
		ИначеЕсли структураОбъект.Наименование = "Оплата труда" Тогда
			ТекущийОбъект = Справочники[ТипСправочника].ОплатаТруда.ПолучитьОбъект();
		ИначеЕсли структураОбъект.Наименование = "Оплата труда (ЕНВД)" Тогда
			ТекущийОбъект = Справочники[ТипСправочника].ОплатаТрудаЕНВД.ПолучитьОбъект();
		ИначеЕсли структураОбъект.Наименование = "Прочие затраты" Тогда
			ТекущийОбъект = Справочники[ТипСправочника].ПрочиеЗатраты.ПолучитьОбъект();
		ИначеЕсли структураОбъект.Наименование = "Содержание служебного автотранспорта" Тогда
			ТекущийОбъект = Справочники[ТипСправочника].СлужебныйАвтотранспорт.ПолучитьОбъект();
		ИначеЕсли структураОбъект.Наименование = "Списание материалов" Тогда
			ТекущийОбъект = Справочники[ТипСправочника].СписаниеМатериалов.ПолучитьОбъект();
		ИначеЕсли структураОбъект.Наименование = "Списание НДС" Тогда
			ТекущийОбъект = Справочники[ТипСправочника].СписаниеНДСНаРасходы.ПолучитьОбъект();
		ИначеЕсли структураОбъект.Наименование = "Списание НДС (ЕНВД)" Тогда
			ТекущийОбъект = Справочники[ТипСправочника].СписаниеНДСНаРасходыЕНВД.ПолучитьОбъект();
		ИначеЕсли структураОбъект.Наименование = "Торговый сбор" Тогда
			ТекущийОбъект = Справочники[ТипСправочника].ТорговыйСбор.ПолучитьОбъект();
		ИначеЕсли структураОбъект.Наименование = "Услуги комиссионеров" Тогда
			ТекущийОбъект = Справочники[ТипСправочника].УслугиКомиссионеров.ПолучитьОбъект();
		Иначе
			ТекущийОбъект = Справочники[ТипСправочника].СоздатьЭлемент();
		КонецЕсли;  
...
Процедура ЗагрузитьЗарплатныйПроект(структураОбъект, ОписаниеОшибок, ЗапрашиваемыеОбъекты) 
	...
	ЗаполнитьРеквизитПоДаннымERP(ТекущийОбъект, "Банк", структураОбъект, "Банк", Тип("СправочникСсылка.КлассификаторБанков"), ЗапрашиваемыеОбъекты, ТекущийОбъект.Банк);
...
Процедура ЗагрузитьАвансовыйОтчет(структураДокумент, ОписаниеОшибок, ЗапрашиваемыеОбъекты)
	...
	Если УИДЗаполнен(структураДокумент.ПодразделениеОрганизации) Тогда
		...
		Иначе
			СтруктураЗапрашиваемогоОбъекта = Новый Структура("УИДВладельца, УИДОбъекта, КлючВСтруктуре, ТипПриемника", структураДокумент.УИД, структураДокумент.ПодразделениеОрганизации, "Подразделение", XmlТипЗнч(ДокументОбъект.ПодразделениеОрганизации).ИмяТипа);
			ЗапрашиваемыеОбъекты.Добавить(СтруктураЗапрашиваемогоОбъекта);
		КонецЕсли;
	Иначе
	...
	Если структураДокумент.Свойство("Прочее") Тогда
		...
		Для Каждого структураСтрока Из структураДокумент.Прочее Цикл
			...
			новСтр.ОтражениеВУСН = Перечисления.ОтражениеВУСН.Принимаются; 
			...
			Если новСтр.СчетЗатрат = ПланыСчетов.Хозрасчетный.ПрочиеРасходы Тогда
				СтатьяРасходов = НайтиОбъектПоИдентификаторуНов(структураСтрока.СтатьяРасходов, Тип("СправочникСсылка.ПрочиеДоходыИРасходы"));
				Если СтатьяРасходов = Неопределено Тогда  
					СтруктураЗапрашиваемогоОбъекта = Новый Структура("УИДВладельца, УИДОбъекта, КлючВСтруктуре, ТипПриемника", "", структураСтрока.СтатьяРасходов, "СтатьяРасходов", XmlТипЗнч(Справочники.ПрочиеДоходыИРасходы.ПустаяСсылка()).ИмяТипа);
					ЗапрашиваемыеОбъекты.Добавить(СтруктураЗапрашиваемогоОбъекта); 
					Продолжить;
				КонецЕсли;
			Иначе
		...
		Для Каждого стрРасхождения Из структураДокумент.Расхождения Цикл
			...
			Для Каждого стрТовары Из ДокументОбъект.Товары Цикл
				Если стрТовары.Номенклатура = Номенклатура
					И стрТовары.Количество + стрРасхождения.Количество >= 0
					И стрТовары.Сумма + Сумма >= 0 Тогда
			...
			Если СтрокаТоваровНайдена Тогда 
				стрТовары.Количество = стрТовары.Количество + стрРасхождения.Количество;
				стрТовары.Сумма = стрТовары.Сумма + Сумма;
				стрТовары.СуммаНДС = стрТовары.СуммаНДС + стрРасхождения.СуммаНДС; 
				
				стрРасхождения.Количество = 0;
				стрРасхождения.Сумма = 0;
				стрРасхождения.СуммаНДС = 0;
			Иначе
				стрРасхождения.Вставить("НоменклатураСсылка", Номенклатура);
			КонецЕсли;
				
		КонецЦикла;  
		
		//распределение оставшегося остатка
		Для каждого стрРасхождения Из структураДокумент.Расхождения Цикл 
			Если стрРасхождения.Количество = 0 Тогда
				Продолжить;
			КонецЕсли;
			...
			Для Каждого стрТовары Из ДокументОбъект.Товары Цикл
				...
				Если стрТовары.Номенклатура <> стрРасхождения.НоменклатураСсылка ИЛИ стрТовары.Количество = 0 Тогда
					Продолжить;
				КонецЕсли; 
				...
				Если стрРасхождения.Количество >= стрТовары.Количество Тогда 
					КоличествоКорректировки = стрТовары.Количество;	
				Иначе                                                        
					КоличествоКорректировки = стрРасхождения.Количество;	
				КонецЕсли;     
				
				СуммаКорректировки = Сумма/стрРасхождения.Количество * КоличествоКорректировки;
				СуммаНДСКорректировки = стрРасхождения.СуммаНДС/стрРасхождения.Количество * КоличествоКорректировки;
				
				стрТовары.Количество = стрТовары.Количество - КоличествоКорректировки;
				стрТовары.Сумма = стрТовары.Сумма - СуммаКорректировки;
				стрТовары.СуммаНДС = стрТовары.СуммаНДС - СуммаНДСКорректировки; 
				
				стрРасхождения.Количество = стрРасхождения.Количество - КоличествоКорректировки;
				стрРасхождения.Сумма = стрРасхождения.Сумма - СуммаКорректировки;
				стрРасхождения.СуммаНДС = стрТовары.СуммаНДС - СуммаНДСКорректировки;
			КонецЦикла; 
		КонецЦикла; 
...
Процедура ЗагрузитьКорректировкуДолга(структураДокумент, ОписаниеОшибок, ЗапрашиваемыеОбъекты)
	...
	Если ЭтоЗагрузкаИзВзаимозачетаЗадолженности Тогда
		...
		//Табличные части  
		ТЧ1 = "ДебиторскаяЗадолженность";
		ТЧ2 = "КредиторскаяЗадолженность";
				
		//ТЧ: ДебиторскаяЗадолженность
		Для Каждого структураСтрока Из структураДокумент[ТЧ1] Цикл	
			ПараметрыДоговора = Новый Структура;
			ПараметрыДоговора.Вставить("ВидДоговора", ?(ТипДебитора = "Клиент", "СПокупателем", "СПоставщиком")); 
			ПараметрыДоговора.Вставить("Валюта", ДокументОбъект.ВалютаДокумента);  
			...
			ТаблицаСчетовИСумм.Колонки.Добавить("Сделка");
			...
			Если НЕ РезультатЗапроса.Пустой() Тогда   
				...
				Пока ВыборкаЗапроса.Следующий() И СуммаВзаимозачетаПоКредиту > 0 Цикл  
					НоваяСтрокаСчета = ТаблицаСчетовИСумм.Добавить();
					НоваяСтрокаСчета.СчетУчетаРасчетов = Структура.СчетУчетаРасчетов; 
					НоваяСтрокаСчета.Сделка = ВыборкаЗапроса.Сделка; 
					СуммаОстаток = ВыборкаЗапроса.СуммаОстаток;
					Если СуммаОстаток < 0 Тогда
						СуммаОстаток = -СуммаОстаток;
					КонецЕсли;
					Если СуммаВзаимозачетаПоКредиту <= СуммаОстаток Тогда
						НоваяСтрокаСчета.СуммаВзаиморасчетов = СуммаВзаимозачетаПоКредиту;
						СуммаВзаимозачетаПоКредиту = 0;
					Иначе
						НоваяСтрокаСчета.СуммаВзаиморасчетов = СуммаОстаток;
						СуммаВзаимозачетаПоКредиту = СуммаВзаимозачетаПоКредиту - СуммаОстаток; 
					КонецЕсли;
				КонецЦикла; 
			...
			Если СуммаВзаимозачетаПоКредиту > 0 Тогда 
				...
				НоваяСтрокаСчета.Сделка = ДокументСсылка;	
				НоваяСтрокаСчета.СуммаВзаиморасчетов = СуммаВзаимозачетаПоКредиту; 
			КонецЕсли;
			
			Для каждого СтрКЗ из ТаблицаСчетовИСумм Цикл
				НоваяСтрока = ДокументОбъект[ТЧ1].Добавить();   
				
				ЗаполнитьЗначенияСвойств(НоваяСтрока, СтрКЗ, "СчетУчетаРасчетов, СуммаВзаиморасчетов, Сделка");
		...
		Для Каждого структураСтрока Из структураДокумент[ТЧ2] Цикл	
			ПараметрыДоговора = Новый Структура;
			...
			Для каждого СтрКЗ из ТаблицаСчетовИСумм Цикл
				НоваяСтрока = ДокументОбъект[ТЧ2].Добавить();   
	...
	ИначеЕсли ЭтоЗагрузкаИзКорректировкиЗадолженности Тогда
		...
		Для Каждого структураСтрока Из структураДокумент.Задолженность Цикл 
			...
			Для каждого Стр из ТаблицаСчетовИСумм Цикл
				...
				ЗаполнитьЗначенияСвойств(НоваяСтрока, Стр, "СчетУчетаРасчетов, СуммаВзаиморасчетов, Сделка");
				НоваяСтрока.ДоговорКонтрагента = ДоговорКонтрагента;
				НоваяСтрока.Сумма = НоваяСтрока.СуммаВзаиморасчетов;
				НоваяСтрока.КурсВзаиморасчетов = 1;	
				НоваяСтрока.КратностьВзаиморасчетов = 1;	
		...
		Если структураДокумент.Свойство("Субконто1") Тогда  
			ЗаполнитьРеквизитПоДаннымERP(ДокументОбъект, "Субконто" + ТипСчета + "1", структураДокумент, "Субконто1", , ЗапрашиваемыеОбъекты);  //Контрагент
		КонецЕсли;	
		Если структураДокумент.Свойство("Субконто2") Тогда	
			ЗаполнитьРеквизитПоДаннымERP(ДокументОбъект, "Субконто" + ТипСчета + "2", структураДокумент, "Субконто2", , ЗапрашиваемыеОбъекты);  //Договор, если есть
		КонецЕсли;
...
Процедура ЗагрузитьСписаниеСРасчетногоСчета(структураДокумент, ОписаниеОшибок, ЗапрашиваемыеОбъекты)
	...
	МассивИП.Добавить(Справочники.Организации.ПолучитьСсылку(Новый УникальныйИдентификатор("a9dee0a0-ed69-11ec-9e16-00155d0b1203"))); //ИП Орлова О. Н.
	МассивИП.Добавить(Справочники.Организации.ПолучитьСсылку(Новый УникальныйИдентификатор("a38ceac4-ed67-11ec-9e16-00155d0b1203"))); //ИП Папсуев А. А.
	
	МассивКонтрагентов = Новый Массив; 
	МассивКонтрагентов.Добавить(Справочники.Контрагенты.НайтиПоНаименованию("Тепло3000", Истина)); 
	
	ЭтоОплатаПоставщикуОтИП = Ложь;
	...
	Если ХозяйственнаяОперация = "ОплатаПоставщику" Тогда 
		...
		ВидДоговора = "СПоставщиком"; 
	ИначеЕсли МассивИП.Найти(ДокументОбъект.Организация) <> Неопределено И МассивКонтрагентов.Найти(ДокументОбъект.Контрагент) <> Неопределено Тогда 
		ДокументОбъект.ВидОперации = Перечисления.ВидыОперацийСписаниеДенежныхСредств.ОплатаПоставщику; 
		ВидДоговора = "СПоставщиком"; 
		ЭтоОплатаПоставщикуОтИП = Истина;
	...
	Если структураДокумент.Свойство("Ведомость") И УИДЗаполнен(структураДокумент.Ведомость) Тогда     
	...
	ДокументОбъект.ОтражениеВУСН = Перечисления.ОтражениеВУСН.ПустаяСсылка();
	...
	Если структураДокумент.Свойство("СчетУчетаИзНастройкиСчетовУчета") Тогда
		...
		Если структураДокумент.Свойство("Субконто1") И ЗначениеЗаполнено(структураДокумент.Субконто1) Тогда
			Если Лев(структураДокумент.ВидСубконто1, 12) = "Перечисление" Тогда
				ДокументОбъект.СубконтоДт1 = Перечисления[Сред(структураДокумент.ВидСубконто1, 14)][структураДокумент.Субконто1];	
			Иначе
				ЗначениеСубконто = НайтиОбъектПоИдентификаторуНов(структураДокумент.Субконто1);
				Если ЗначениеСубконто <> Неопределено Тогда
					ДокументОбъект.СубконтоДт1 = ЗначениеСубконто;	
				Иначе
					СтруктураЗапрашиваемогоОбъекта = Новый Структура("УИДВладельца, УИДОбъекта, КлючВСтруктуре, ТипПриемника", "", структураДокумент.Субконто1, "", "");
					ЗапрашиваемыеОбъекты.Добавить(СтруктураЗапрашиваемогоОбъекта);					
				КонецЕсли;
			КонецЕсли;
		КонецЕсли;  
		
		Если ТипЗнч(ДокументОбъект.СубконтоДт1) = Тип("ПеречислениеСсылка.ВидыПлатежейВГосБюджет") Тогда
			ДокументОбъект.ВидНалоговогоОбязательства = ДокументОбъект.СубконтоДт1;
			//Савина Н. Начало 16.11.2022 
			Если НЕ ЗначениеЗаполнено(ДокументОбъект.ВидНалоговогоОбязательства) Тогда 
				ДокументОбъект.ВидНалоговогоОбязательства = Перечисления.ВидыПлатежейВГосБюджет.Налог;	
			КонецЕсли;
			//Савина Н. Конец 16.11.2022
		ИначеЕсли ТипЗнч(ДокументОбъект.СубконтоДт1) = Тип("СправочникСсылка.ПрочиеДоходыИРасходы") Тогда
			Если ДокументОбъект.СубконтоДт1.Наименование = "Пени, штрафы (непринимаемые)" Тогда
				ДокументОбъект.ВидНалоговогоОбязательства = Перечисления.ВидыПлатежейВГосБюджет.Штраф;
			КонецЕсли;
		КонецЕсли;
		
		Если структураДокумент.Свойство("Субконто2") И ЗначениеЗаполнено(структураДокумент.Субконто2) Тогда
			Если Лев(структураДокумент.ВидСубконто2, 12) = "Перечисление" Тогда
				ДокументОбъект.СубконтоДт2 = Перечисления[Сред(структураДокумент.ВидСубконто2, 14)][структураДокумент.Субконто2];	
			Иначе
				ЗначениеСубконто = НайтиОбъектПоИдентификаторуНов(структураДокумент.Субконто2);
				Если ЗначениеСубконто <> Неопределено Тогда
					ДокументОбъект.СубконтоДт2 = ЗначениеСубконто;	
				Иначе
					СтруктураЗапрашиваемогоОбъекта = Новый Структура("УИДВладельца, УИДОбъекта, КлючВСтруктуре, ТипПриемника", "", структураДокумент.Субконто2, "", "");
					ЗапрашиваемыеОбъекты.Добавить(СтруктураЗапрашиваемогоОбъекта);					
				КонецЕсли;
			КонецЕсли;
		КонецЕсли;
	...
	ИначеЕсли структураДокумент.Свойство("СчетУчетаРасчетовСКонтрагентом") Тогда  
		...
		Если ДокументОбъект.СчетУчетаРасчетовСКонтрагентом = ПланыСчетов.Хозрасчетный.ПрочиеРасходы Тогда 
			...
			ДокументОбъект.Графа6_УСН = структураДокумент.СуммаДокумента;  
	...
	Для Каждого структураСтрока Из структураДокумент.РасшифровкаПлатежа Цикл
		...
		Если НЕ ЭтоПрочееСписание Тогда
			...
			ЗаполнитьРеквизитПоДаннымERP(НоваяСтрока, "СтатьяДвиженияДенежныхСредств", структураСтрока, "СтатьяДвиженияДенежныхСредств", , ЗапрашиваемыеОбъекты);  
			Если НЕ ЗначениеЗаполнено(НоваяСтрока.СтатьяДвиженияДенежныхСредств) ИЛИ ЭтоОплатаПоставщикуОтИП Тогда  
				НоваяСтрока.СтатьяДвиженияДенежныхСредств = УчетДенежныхСредствБП.СтатьяДДСПоУмолчанию(ДокументОбъект.ВидОперации);
			КонецЕсли;   
			Если НоваяСтрока.СтатьяДвиженияДенежныхСредств = Справочники.СтатьиДвиженияДенежныхСредств.НайтиПоНаименованию("Лизинговые платежи", Истина) Тогда    
				НоваяСтрока.СчетУчетаРасчетовСКонтрагентом = ПланыСчетов.Хозрасчетный.АрендныеПлатежи;
			КонецЕсли;
			...
			НоваяСтрока.СпособПогашенияЗадолженности = Перечисления.СпособыПогашенияЗадолженности.Автоматически;   
			НоваяСтрока.ОтражениеВУСН = Перечисления.ОтражениеВУСН.ПустаяСсылка();  
		КонецЕсли;
		
	КонецЦикла;	 
	
	Если ЭтоОплатаПоставщикуОтИП Тогда  
...
Процедура ЗагрузитьПриобретениеТоваровУслуг(структураДокумент, ОписаниеОшибок, ЗапрашиваемыеОбъекты) 
	...
	ДокументОбъект.СчетУчетаРасчетовСКонтрагентом = ПланыСчетов.Хозрасчетный.ПустаяСсылка();
	...
	КонецЕсли;
	ЗаполнитьРеквизитПоДаннымERP(ДокументОбъект, "Организация",структураДокумент, "Организация", Тип("СправочникСсылка.Организации"), ЗапрашиваемыеОбъекты, Справочники.Организации.ПустаяСсылка());
	ЗаполнитьРеквизитПоДаннымERP(ДокументОбъект, "Склад", структураДокумент, "Склад", Тип("СправочникСсылка.Склады"), ЗапрашиваемыеОбъекты);
	...
	Если ДокументОснование.Дата >= '20210101' Тогда
		...
		Для каждого стрРасхождения Из структураДокумент.Расхождения Цикл 
			...
			Для Каждого стрТовары Из ДокументОбъект.Товары Цикл
				...
				СуммаКорректировки = Окр(Сумма/стрРасхождения.Количество, 2, 1) * КоличествоКорректировки;
				СуммаНДСКорректировки = Окр(стрРасхождения.СуммаНДС/стрРасхождения.Количество, 2, 1) * КоличествоКорректировки;
				...
				стрРасхождения.Количество = стрРасхождения.Количество - КоличествоКорректировки;
				стрРасхождения.Сумма = стрРасхождения.Сумма - СуммаКорректировки;
				стрРасхождения.СуммаНДС = стрРасхождения.СуммаНДС - СуммаНДСКорректировки;
			КонецЦикла; 
		КонецЦикла; 
...
Процедура ЗагрузитьНачислениеДивидендов(структураДокумент, ОписаниеОшибок, ЗапрашиваемыеОбъекты)
	
	ДокументОбъект = НайтиСоздатьОбъектДокумента(структураДокумент.УИД,"НачислениеДивидендов");                                    
	...
	ДокументОбъект.РасчетныйПериод = ДокументОбъект.Дата;
	ДокументОбъект.ТипУчредителя = Перечисления.ЮридическоеФизическоеЛицо.ФизическоеЛицо;
	ЗаполнитьРеквизитПоДаннымERP(ДокументОбъект, "Учредитель",структураДокумент, "Учредитель", , ЗапрашиваемыеОбъекты);
	ДокументОбъект.СуммаДохода = структураДокумент.СуммаДохода;

	ДокументОбъект.СуммаНалога = структураДокумент.СуммаНалога;   
	
	ДокументОбъект.ПланируемаяДатаВыплаты = структураДокумент.ПланируемаяДатаВыплаты;
...
Процедура ЗагрузитьОбъектПоСтруктуре(структураОбъект, ОписаниеОшибок, ЗапрашиваемыеОбъекты)
	...
	ИначеЕсли структураОбъект.ТипОбъекта = "НачислениеДивидендов" Тогда
		ЗагрузитьНачислениеДивидендов(структураОбъект, ОписаниеОшибок, ЗапрашиваемыеОбъекты);   
