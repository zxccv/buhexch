...
Процедура ЗагрузитьВозвратТоваровПоставщику(структураДокумент, ОписаниеОшибок, ЗапрашиваемыеОбъекты) 
	...
	Если ДокументСсылка <> Неопределено И ОбщегоНазначения.СсылкаСуществует(ДокументСсылка) Тогда
		...
		Если ДокументОбъект.ПометкаУдаления <> структураДокумент.ПометкаУдаления Тогда
			ДокументОбъект.УстановитьПометкуУдаления(структураДокумент.ПометкаУдаления);
		КонецЕсли;
	...
	//ЗаполнитьРеквизитПоДаннымERP(ДокументОбъект, "Ответственный", структураДокумент, "Менеджер", , ЗапрашиваемыеОбъекты);
