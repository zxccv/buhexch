...
Функция НайтиСоздатьОбъектДокумента(УИДДокумента, ТипДокумента)
	...
	ДокументСсылка = НайтиОбъектПоИдентификаторуНов(УИДДокумента);	
...
//  НадоДобавитьЗаписьВРегистр	 - 	 Тип: Булево. Признак необходимости формирования новой записи в регистре сведений ПубличныеИдентификаторыСинхронизируемыхОбъектов. 
// 
...
//
Функция ПолучитьОбъектСправочника(структураОбъект, ТипСправочника, НадоДобавитьЗаписьВРегистр) 
	...
	Если ИскомаяСсылка = Неопределено Тогда
		Если структураОбъект.ЭтоГруппа Тогда
			СсылкаПоКоду = Справочники[ТипСправочника].НайтиПоКоду(структураОбъект.Код);
			...
			Иначе
				ТекущийОбъект = СсылкаПоКоду.ПолучитьОбъект();
				ЗаписатьИдентификаторВРегистр(СсылкаПоКоду, структураОбъект.УИД);		
			КонецЕсли;
		Иначе	
...
Процедура ЗагрузитьСтатьюЗатрат(структураОбъект, ОписаниеОшибок, ЗапрашиваемыеОбъекты)
	...
	Если ЗначениеЗаполнено(структураОбъект.ВидРасходовНУ) Тогда
    	СправочникОбъект.ВидРасходовНУ = Перечисления.ВидыРасходовНУ[структураОбъект.ВидРасходовНУ];
	КонецЕсли;	
...
Процедура ЗагрузитьБанк(структураОбъект, ОписаниеОшибок, ЗапрашиваемыеОбъекты)
	...
	БанкОбъект = ПолучитьОбъектСправочника(структураОбъект, ТипСправочника, НадоДобавитьЗаписьВРегистр);  
...
Процедура ЗагрузитьКлассификаторБанка(структураОбъект, ОписаниеОшибок, ЗапрашиваемыеОбъекты)
	...
	ТекущийОбъект = ПолучитьОбъектСправочника(структураОбъект, ТипСправочника, НадоДобавитьЗаписьВРегистр);  
	ТекущийОбъект.ОбменДанными.Загрузка = Истина;

	ЗаполнитьЗначенияСвойств(ТекущийОбъект, структураОбъект);
	
	Если УИДЗаполнен(структураОбъект.Родитель) Тогда
		ТекущийОбъект.Родитель = НайтиОбъектПоИдентификаторуНов(структураОбъект.Родитель, Тип("СправочникСсылка.КлассификаторБанков"));
...
Процедура ЗагрузитьПриходныйКассовыйОрдер(структураДокумент, ОписаниеОшибок, ЗапрашиваемыеОбъекты) 
	...
	Для Каждого структураСтрока Из структураДокумент.РасшифровкаПлатежа Цикл	
		...
		//ТЧ: Статья движения денежных средств
...
Процедура ЗагрузитьСписаниеСРасчетногоСчета(структураДокумент, ОписаниеОшибок, ЗапрашиваемыеОбъекты)
	...
	Для Каждого структураСтрока Из структураДокумент.РасшифровкаПлатежа Цикл
		...
		//прочее списание 
		Если структураСтрока.Свойство("СчетСтатьиРасходов") Тогда
			//СчетУчетаРасчетовСКонтрагентом
			ЗаполнитьРеквизитПоДаннымERP(ДокументОбъект, "СчетУчетаРасчетовСКонтрагентом", структураСтрока, "СчетСтатьиРасходов", , ЗапрашиваемыеОбъекты); 
			ДокументОбъект.ОтражениеВУСН = Перечисления.ОтражениеВУСН.НеПринимаются; 
			
			//прочие доходы и расходы    
			Если СтрНайти(структураСтрока.СтатьяРасходов, "Услуги банка") > 0  
				ИЛИ структураСтрока.СтатьяРасходов = "Финансирование за счет кредитов и займов" Тогда
		...
		Иначе
			...
			//ТР: Статья движения денежных средств
			ЗаполнитьРеквизитПоДаннымERP(НоваяСтрока, "СтатьяДвиженияДенежныхСредств", структураСтрока, "СтатьяДвиженияДенежныхСредств", , ЗапрашиваемыеОбъекты);  
			Если НЕ ЗначениеЗаполнено(НоваяСтрока.СтатьяДвиженияДенежныхСредств) Тогда  
				НоваяСтрока.СтатьяДвиженияДенежныхСредств = УчетДенежныхСредствБП.СтатьяДДСПоУмолчанию(ДокументОбъект.ВидОперации);
			КонецЕсли;
...
Процедура ЗагрузитьВедомостьНаВыплатуЗарплатыВБанк(структураДокумент, ОписаниеОшибок, ЗапрашиваемыеОбъекты) 
	...
	ДокументОбъект.ПериодРегистрации = Дата(Лев(структураДокумент.ПериодРегистрации, 4), Сред(структураДокумент.ПериодРегистрации, 6, 2), Сред(структураДокумент.ПериодРегистрации, 9, 2));
	...
	//Перечисление: ВидДоходаИсполнительногоПроизводства
	...
	Для Каждого структураСтрока Из структураДокумент.Зарплата Цикл
		...
		НоваяСтрока.ПериодВзаиморасчетов = Дата(Лев(структураСтрока.ПериодВзаиморасчетов, 4), Сред(структураСтрока.ПериодВзаиморасчетов, 6, 2), Сред(структураСтрока.ПериодВзаиморасчетов, 9, 2));
	...
	Для Каждого структураСтрока Из структураДокумент.НДФЛ Цикл
		...
		//ФизическоеЛицо
		ЗаполнитьРеквизитПоДаннымERP(НоваяСтрока, "ФизическоеЛицо", структураСтрока, "ФизическоеЛицо", Тип("СправочникСсылка.ФизическиеЛица"), ЗапрашиваемыеОбъекты); 
		//Перечисление: СтавкаНалогообложенияРезидента
		...
		НоваяСтрока.МесяцНалоговогоПериода = Дата(Лев(структураСтрока.МесяцНалоговогоПериода, 4), Сред(структураСтрока.МесяцНалоговогоПериода, 6, 2), Сред(структураСтрока.МесяцНалоговогоПериода, 9, 2));
