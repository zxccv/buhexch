...
Функция НайтиОбъектПоИдентификатору(УИД, ВернутьПервыйЭлемент = Ложь)
	
	//!! убрать лишние проверки перед вызовами поиска
...
Функция НайтиОрганизацию(НайденныеОбъекты)
	Возврат НайтиОбъектПоТипу(НайденныеОбъекты, Тип("СправочникСсылка.Организации"));	
КонецФункции

Функция НайтиКонтрагента(НайденныеОбъекты) 
	Возврат НайтиОбъектПоТипу(НайденныеОбъекты, Тип("СправочникСсылка.Контрагенты"));	
КонецФункции

Функция НайтиОбъектСтроительства(НайденныеОбъекты) 
	Возврат НайтиОбъектПоТипу(НайденныеОбъекты, Тип("СправочникСсылка.ОбъектыСтроительства"));	
КонецФункции
...
Процедура ЗаполнитьНомерГТД(Нстр, структураГТД, ЗапрашиваемыеОбъекты, УИДВладельца)
	...
	Если УИДЗаполнен(структураГТД.СтранаПроисхождения) Тогда
		Страна = НайтиОбъектПоИдентификатору(структураГТД.СтранаПроисхождения, Истина);
...
Функция НайтиОбъектПоТипу(НайденныеОбъекты , Тип)
	...
	Для Каждого Стр Из НайденныеОбъекты Цикл
		Если ТипЗнч(Стр.Ссылка) = Тип Тогда
			Возврат Стр.Ссылка;
		КонецЕсли;			
	КонецЦикла;	
...
Процедура ЗагрузитьВалюту(структураОбъект, ОписаниеОшибок, ЗапрашиваемыеОбъекты)
	...
	НайденныеОбъекты = НайтиОбъектПоИдентификатору(структураОбъект.УИД);
	Если НайденныеОбъекты <> Неопределено И Не структураОбъект.ОбъектИзменен Тогда
		Возврат;
	ИначеЕсли НайденныеОбъекты = Неопределено Тогда
		ВалютаОбъект = Справочники.Валюты.СоздатьЭлемент();
		НадоДобавитьЗаписьВРегистр = Истина;
	Иначе
		ВалютаОбъект = НайденныеОбъекты[0].Ссылка.ПолучитьОбъект();	
	КонецЕсли;	
	...
	Если структураОбъект.СпособУстановкиКурса = "НаценкаНаКурсДругойВалюты" Тогда
		...
		ВалютаОбъект.ОсновнаяВалюта = НайтиОбъектПоИдентификатору(структураОбъект.ОсновнаяВалюта, Истина);
	...
	Если НадоДобавитьЗаписьВРегистр Тогда
		ЗаписатьИдентификаторВРегистр(ВалютаОбъект.Ссылка, структураОбъект.УИД);
	КонецЕсли;	
...
Процедура ЗагрузитьОрганизацию(структураОбъект, ОписаниеОшибок, ЗапрашиваемыеОбъекты)
	...
	Иначе   
		Для Каждого Стр Из НайденныеОбъекты Цикл
			Если ТипЗнч(Стр.Ссылка) = Тип("СправочникСсылка.Организации") Тогда
				ОрганизацияОбъект = Стр.Ссылка.ПолучитьОбъект();
				ОрганизацияНайдена = Истина;
				Прервать;
			КонецЕсли;		
		КонецЦикла;	  
		Если Не ОрганизацияНайдена Тогда
			ОрганизацияОбъект = Справочники.Организации.СоздатьЭлемент();				
			НадоДобавитьЗаписьВРегистр = Истина;			
		КонецЕсли;	
	КонецЕсли;	
	
	Если ОрганизацияНайдена И Не структураОбъект.ОбъектИзменен Тогда
		Возврат;
	КонецЕсли;	
	...
	Если НадоДобавитьЗаписьВРегистр Тогда     
		...
		ОрганизацияОбъект.УстановитьСсылкуНового(СсылкаНового);
		ОрганизацияОбъект.ГоловнаяОрганизация = СсылкаНового;
	...
	Если структураОбъект.Свойство("ГоловнаяОрганизация") И УИДЗаполнен(структураОбъект.ГоловнаяОрганизация) Тогда
		НайденныеОбъекты = НайтиОбъектПоИдентификатору(структураОбъект.ГоловнаяОрганизация);
		ОбъектНайден = Ложь;
		Для Каждого Стр Из НайденныеОбъекты Цикл
			Если ТипЗнч(Стр.Ссылка) = Тип("СправочникСсылка.Организации") Тогда
				ОрганизацияОбъект.ГоловнаяОрганизация = Стр.Ссылка;		
				ОбъектНайден = Истина;
				Прервать;
			КонецЕсли;
		КонецЦикла;	  
	...
	Если УИДЗаполнен(структураОбъект.ИндивидуальныйПредприниматель)Тогда
		ОрганизацияОбъект.ИндивидуальныйПредприниматель = НайтиОбъектПоИдентификатору(структураОбъект.ИндивидуальныйПредприниматель, Истина);
		Если Не ЗначениеЗаполнено(ОрганизацияОбъект.ИндивидуальныйПредприниматель) Тогда
			//ОписаниеОшибок = ОписаниеОшибок + Символы.ПС + "По УИД " + структураОбъект.ГоловнаяОрганизация + " не удалось найти физ.лицо ИП для организации " + ОрганизацияОбъект.НаименованиеСокращенное;
	...
	Если УИДЗаполнен(структураОбъект.РегистрацияВНалоговомОргане)Тогда
		ОрганизацияОбъект.РегистрацияВНалоговомОргане = НайтиОбъектПоИдентификатору(структураОбъект.РегистрацияВНалоговомОргане, Истина);
		Если Не ЗначениеЗаполнено(ОрганизацияОбъект.РегистрацияВНалоговомОргане) Тогда
			//ОписаниеОшибок = ОписаниеОшибок + Символы.ПС + "По УИД " + структураОбъект.РегистрацияВНалоговомОргане + " не удалось найти регистрацию в налоговом органе для организации " + ОрганизацияОбъект.НаименованиеСокращенное;
	...
	Если УИДЗаполнен(структураОбъект.УчетнаяЗаписьОбмена)Тогда
		ОрганизацияОбъект.УчетнаяЗаписьОбмена = НайтиОбъектПоИдентификатору(структураОбъект.УчетнаяЗаписьОбмена, Истина);
	...
	Если НадоДобавитьЗаписьВРегистр Тогда
		ЗаписатьИдентификаторВРегистр(ОрганизацияОбъект.Ссылка, структураОбъект.УИД);
	КонецЕсли;	
...
Процедура ЗагрузитьПодразделение(структураОбъект, ОписаниеОшибок, ЗапрашиваемыеОбъекты)
	...
	Если УИДЗаполнен(структураОбъект.Организация) Тогда
		...
		Организация = НайтиОрганизацию(НайденныеОбъекты);
	...
	НайденныеОбъекты = НайтиОбъектПоИдентификатору(структураОбъект.УИД);
	НеНадоСоздатьНовыйОбъект = Ложь;
	Если НайденныеОбъекты <> Неопределено Тогда
		
		Для Каждого Стр Из НайденныеОбъекты Цикл
			
			Если Организация <> Неопределено И Стр.Ссылка.Владелец = Организация Тогда
				НеНадоСоздатьНовыйОбъект = Истина;	
			КонецЕсли;	
			
			Если структураОбъект.ОбъектИзменен Тогда
				мсвПодразделений.Добавить(Стр.Ссылка);
			КонецЕсли;
			
		КонецЦикла;	                              	
		
		Если НеНадоСоздатьНовыйОбъект = Ложь Тогда  
			мсвПодразделений.Добавить(Справочники.ПодразделенияОрганизаций.ПустаяСсылка());	
		КонецЕсли;			
	...
	Для каждого Эл из мсвПодразделений Цикл
				
		НадоДобавитьЗаписьВРегистр = Ложь;

		Если Эл.Пустая() Тогда
			ПодразделениеОбъект = Справочники.ПодразделенияОрганизаций.СоздатьЭлемент();
			ПодразделениеОбъект.НаименованиеПолное = структураОбъект.Наименование;
			НадоДобавитьЗаписьВРегистр = Истина;
			ПодразделениеОбъект.Владелец = Организация;	
		Иначе
			ПодразделениеОбъект = Эл.ПолучитьОбъект(); 
		КонецЕсли;	
		
		ТекОрганизация = ПодразделениеОбъект.Владелец;
		...
		Если УИДЗаполнен(структураОбъект.Родитель) Тогда
			НайденныеОбъекты = НайтиОбъектПоИдентификатору(структураОбъект.Родитель);
			РодительНайден = Ложь;
			Если НайденныеОбъекты <> Неопределено Тогда
				Для каждого Стр из НайденныеОбъекты Цикл
					Если Стр.Ссылка.Владелец = ТекОрганизация Тогда
						ПодразделениеОбъект.Родитель = Стр.Ссылка;
		...
		Если ЗначениеЗаполнено(ПодразделениеОбъект.Владелец) Тогда		
			...
			Если НадоДобавитьЗаписьВРегистр Тогда
				ЗаписатьИдентификаторВРегистр(ПодразделениеОбъект.Ссылка, структураОбъект.УИД);
			КонецЕсли;
...
Процедура ЗагрузитьРегистрациюВНалоговомОргане(структураОбъект, ОписаниеОшибок, ЗапрашиваемыеОбъекты)
	...
	ИначеЕсли НайденныеОбъекты = Неопределено Тогда
		...
		НадоДобавитьЗаписьВРегистр = Истина;
	Иначе
		РегистрацияОбъект = НайденныеОбъекты[0].Ссылка.ПолучитьОбъект();	
	КонецЕсли;	
	...
	НайденныеОбъекты = НайтиОбъектПоИдентификатору(структураОбъект.Владелец);
	Если НайденныеОбъекты <> Неопределено Тогда
		РегистрацияОбъект.Владелец = НайтиОрганизацию(НайденныеОбъекты);
		Если НЕ ЗначениеЗаполнено(РегистрацияОбъект.Владелец) Тогда
		...
		КонецЕсли;	
	Иначе     
		//ошибка что не найден владелец
		//!! едва ли будет использоваться тут, просто для примера
		НовОрг = Справочники.Организации.СоздатьЭлемент();
		//ОргНоваяСсылка = Справочники.Организации.ПолучитьСсылку();
		//НовОрг.УстановитьСсылкуНового(ОргНоваяСсылка);
		НовОрг.ОбменДанными.Загрузка = Истина;
		//возможно потребуется заполнение ряда реквизитов
		НовОрг.Записать();
		РегистрацияОбъект.Владелец = НовОрг;
		ЗаписатьИдентификаторВРегистр(НовОрг.Ссылка, структураОбъект.Владелец);
	...
	Если НадоДобавитьЗаписьВРегистр Тогда
		ЗаписатьИдентификаторВРегистр(РегистрацияОбъект.Ссылка, структураОбъект.УИД);
	КонецЕсли;	
...
Процедура ЗагрузитьУчетнуюЗаписьДокументооборота(структураОбъект, ОписаниеОшибок, ЗапрашиваемыеОбъекты)
	...
	ИначеЕсли НайденныеОбъекты = Неопределено Тогда
		...
		НадоДобавитьЗаписьВРегистр = Истина;
	Иначе
		УчетнуюЗаписьОбъект = НайденныеОбъекты[0].Ссылка.ПолучитьОбъект();	
	КонецЕсли;
	...
	Если УИДЗаполнен(структураОбъект.НалоговыйОрган) Тогда
		НалоговыйОрган = НайтиОбъектПоИдентификатору(структураОбъект.НалоговыйОрган, Истина);
		Если НалоговыйОрган <> Неопределено Тогда                  
			УчетнаяЗаписьОбъект = НалоговыйОрган;
		Иначе
		...
		КонецЕсли;
	Иначе
		УчетнаяЗаписьОбъект = Справочники.НалоговыеОрганы.ПустаяСсылка();
	КонецЕсли;		
	
	Если УИДЗаполнен(структураОбъект.СерверДокументооборота) Тогда
		СерверДокументооборота = НайтиОбъектПоИдентификатору(структураОбъект.СерверДокументооборота, Истина);
		...
		КонецЕсли;
	Иначе
		УчетнаяЗаписьОбъект.СерверДокументооборота = Справочники.СерверыДокументооборота.ПустаяСсылка();
	КонецЕсли;	
	...
	Если НадоДобавитьЗаписьВРегистр Тогда
		ЗаписатьИдентификаторВРегистр(УчетнаяЗаписьОбъект.Ссылка, структураОбъект.УИД);
	КонецЕсли;	
...
Процедура ЗагрузитьСерверДокументооборота(структураОбъект, ОписаниеОшибок, ЗапрашиваемыеОбъекты)
	...
	ИначеЕсли НайденныеОбъекты = Неопределено Тогда
		...
		НадоДобавитьЗаписьВРегистр = Истина;
	Иначе
		СерверДокументооборотаОбъект = НайденныеОбъекты[0].Ссылка.ПолучитьОбъект();	
	КонецЕсли;
	...
	Если НадоДобавитьЗаписьВРегистр Тогда
		ЗаписатьИдентификаторВРегистр(СерверДокументооборотаОбъект.Ссылка, структураОбъект.УИД);
	КонецЕсли;	
...
Процедура ЗагрузитьНалоговыйОрган(структураОбъект, ОписаниеОшибок, ЗапрашиваемыеОбъекты)
	...
	ИначеЕсли НайденныеОбъекты = Неопределено Тогда
		...
		НадоДобавитьЗаписьВРегистр = Истина;
	Иначе
		НалоговыйОрганОбъект = НайденныеОбъекты[0].Ссылка.ПолучитьОбъект();	
	КонецЕсли;
	...
	Если УИДЗаполнен(структураОбъект.Родитель) Тогда
		НайденныеОбъекты = НайтиОбъектПоИдентификатору(структураОбъект.Родитель);
		Если НайденныеОбъекты <> Неопределено Тогда
			НалоговыйОрганОбъект.Родитель = НайденныеОбъекты[0].Ссылка;
		Иначе
	...
	Если НадоДобавитьЗаписьВРегистр Тогда
		ЗаписатьИдентификаторВРегистр(НалоговыйОрганОбъект.Ссылка, структураОбъект.УИД);
	КонецЕсли;	
...
Процедура ЗагрузитьКонтрагента(структураОбъект, ОписаниеОшибок, ЗапрашиваемыеОбъекты)
	...
	Иначе
		Для Каждого Стр Из НайденныеОбъекты Цикл
			Если ТипЗнч(Стр.Ссылка) = Тип("СправочникСсылка.Контрагенты") Тогда
	...
	Если КонтрагентНайден И Не структураОбъект.ОбъектИзменен Тогда
		Возврат;
	КонецЕсли;	
	...
	Если НадоДобавитьЗаписьВРегистр Тогда     
		...
		КонтрагентОбъект.УстановитьСсылкуНового(СсылкаНового);
		КонтрагентОбъект.ГоловнойКонтрагент = СсылкаНового;
	Иначе
	...
	Если структураОбъект.Свойство("ГоловнойКонтрагент") И УИДЗаполнен(структураОбъект.ГоловнойКонтрагент) Тогда
		НайденныеОбъекты = НайтиОбъектПоИдентификатору(структураОбъект.ГоловнойКонтрагент);
		КонтрагентОбъект.ГоловнойКонтрагент = НайтиКонтрагента(НайденныеОбъекты);
		Если Не ЗначениеЗаполнено(КонтрагентОбъект.ГоловнойКонтрагент) Тогда
	...
	Если УИДЗаполнен(структураОбъект.СтранаРегистрации) Тогда
		КонтрагентОбъект.СтранаРегистрации = НайтиОбъектПоИдентификатору(структураОбъект.СтранаРегистрации, Истина);
	...
	Если НадоДобавитьЗаписьВРегистр Тогда
		ЗаписатьИдентификаторВРегистр(КонтрагентОбъект.Ссылка, структураОбъект.УИД);
	КонецЕсли;	
...
Процедура ЗагрузитьДоговор(структураОбъект, ОписаниеОшибок, ЗапрашиваемыеОбъекты)
	...
	ИначеЕсли НайденныеОбъекты = Неопределено Тогда
		...
		НадоДобавитьЗаписьВРегистр = Истина;
	Иначе
		ДоговорОбъект = НайденныеОбъекты[0].Ссылка.ПолучитьОбъект();  
	КонецЕсли;
	...
	Если УИДЗаполнен(структураОбъект.ВалютаВзаиморасчетов) Тогда
		ДоговорОбъект.ВалютаВзаиморасчетов = НайтиОбъектПоИдентификатору(структураОбъект.ВалютаВзаиморасчетов, Истина);
	...
	Если УИДЗаполнен(структураОбъект.Организация) Тогда
		НайденныеОбъекты = НайтиОбъектПоИдентификатору(структураОбъект.Организация);
		ДоговорОбъект.Организация = НайтиОрганизацию(НайденныеОбъекты);
	...
	Если УИДЗаполнен(структураОбъект.Контрагент) Тогда
		НайденныеОбъекты = НайтиОбъектПоИдентификатору(структураОбъект.Контрагент);
		ДоговорОбъект.Владелец = НайтиКонтрагента(НайденныеОбъекты);
		Если Не ЗначениеЗаполнено(ДоговорОбъект.Организация) Тогда
	...
	Если НадоДобавитьЗаписьВРегистр Тогда
		ЗаписатьИдентификаторВРегистр(ДоговорОбъект.Ссылка, структураОбъект.УИД);
	КонецЕсли;  
...
Процедура ЗагрузитьСтрану(структураОбъект, ОписаниеОшибок, ЗапрашиваемыеОбъекты)
	...
	ИначеЕсли НайденныеОбъекты = Неопределено Тогда
		...
		НадоДобавитьЗаписьВРегистр = Истина;
	Иначе
		СтранаОбъект = НайденныеОбъекты[0].Ссылка.ПолучитьОбъект();	
	КонецЕсли;
	...
	Если НадоДобавитьЗаписьВРегистр Тогда
		ЗаписатьИдентификаторВРегистр(СтранаОбъект.Ссылка, структураОбъект.УИД);
	КонецЕсли;	
...
Процедура ЗагрузитьНомеклатуру(структураОбъект, ОписаниеОшибок, ЗапрашиваемыеОбъекты)
	...
	ИначеЕсли НайденныеОбъекты = Неопределено Тогда
		...
		НадоДобавитьЗаписьВРегистр = Истина;
	Иначе
		НоменклатураОбъект = НайденныеОбъекты[0].Ссылка.ПолучитьОбъект();	
	КонецЕсли;
	...
	Если УИДЗаполнен(структураОбъект.Родитель) Тогда
		НоменклатураОбъект.Родитель = НайтиОбъектПоИдентификатору(структураОбъект.Родитель, Истина);
	...
	Если Не структураОбъект.ЭтоГруппа Тогда
		...
		Если УИДЗаполнен(структураОбъект.ЕдиницаИзмерения) Тогда             
			НоменклатураОбъект.ЕдиницаИзмерения = НайтиОбъектПоИдентификатору(структураОбъект.ЕдиницаИзмерения, Истина);
	...
	Если НадоДобавитьЗаписьВРегистр Тогда
		ЗаписатьИдентификаторВРегистр(НоменклатураОбъект.Ссылка, структураОбъект.УИД);
	КонецЕсли;	
...
Процедура ЗагрузитьЕдиницуИзмерения(структураОбъект, ОписаниеОшибок, ЗапрашиваемыеОбъекты)
	...
	ИначеЕсли НайденныеОбъекты = Неопределено Тогда
		...
		НадоДобавитьЗаписьВРегистр = Истина;
	Иначе
		ЕдИзмОбъект = НайденныеОбъекты[0].Ссылка.ПолучитьОбъект();	
	КонецЕсли;
	...
	Если НадоДобавитьЗаписьВРегистр Тогда
		ЗаписатьИдентификаторВРегистр(ЕдИзмОбъект.Ссылка, структураОбъект.УИД);
	КонецЕсли;	
...
Процедура ЗагрузитьСклад(структураОбъект, ОписаниеОшибок, ЗапрашиваемыеОбъекты)
	...
	ИначеЕсли НайденныеОбъекты = Неопределено Тогда
		...
		НадоДобавитьЗаписьВРегистр = Истина;
	Иначе
		СкладОбъект = НайденныеОбъекты[0].Ссылка.ПолучитьОбъект();	
	КонецЕсли;
	...
	Если УИДЗаполнен(структураОбъект.Родитель) Тогда
		СкладОбъект.Родитель = НайтиОбъектПоИдентификатору(структураОбъект.Родитель, Истина);
	...
	Если НадоДобавитьЗаписьВРегистр Тогда
		ЗаписатьИдентификаторВРегистр(СкладОбъект.Ссылка, структураОбъект.УИД);
	КонецЕсли;		
...
Процедура ЗагрузитьРасходыБудущихПериодов(структураОбъект, ОписаниеОшибок, ЗапрашиваемыеОбъекты)
	...
	//? возможно, нужно учесть, что загружается из статей расходов, которые могут принимать разные типы значений в БП, т.е. гипотетически может быть несколько ссылок на один УИД
	НадоДобавитьЗаписьВРегистр = Ложь;
	НайденныеОбъекты = НайтиОбъектПоИдентификатору(структураОбъект.УИД);
	Если НайденныеОбъекты <> Неопределено И Не структураОбъект.ОбъектИзменен Тогда
		Возврат;
	ИначеЕсли НайденныеОбъекты = Неопределено Тогда
		...
		НадоДобавитьЗаписьВРегистр = Истина;
	Иначе
		РБПОбъект = НайденныеОбъекты[0].Ссылка.ПолучитьОбъект();	
	КонецЕсли;
	...
	Если НадоДобавитьЗаписьВРегистр Тогда
		ЗаписатьИдентификаторВРегистр(РБПОбъект.Ссылка, структураОбъект.УИД);
	КонецЕсли;		
...
Процедура ЗагрузитьПрочиеДоходыИРасходы(структураОбъект, ОписаниеОшибок, ЗапрашиваемыеОбъекты)
	...
	ИначеЕсли НайденныеОбъекты = Неопределено Тогда
		...
		КонецЕсли;	
	Иначе
		ПДРОбъект = НайденныеОбъекты[0].Ссылка.ПолучитьОбъект();	
	КонецЕсли;
...
Процедура ЗагрузитьФизЛицо(структураОбъект, ОписаниеОшибок, ЗапрашиваемыеОбъекты)
	...
	ИначеЕсли НайденныеОбъекты = Неопределено Тогда
		...
		НадоДобавитьЗаписьВРегистр = Истина;
	Иначе
		ФизЛицоОбъект = НайденныеОбъекты[0].Ссылка.ПолучитьОбъект();	
	КонецЕсли;
	...
	Если УИДЗаполнен(структураОбъект.Родитель) Тогда
		ФизЛицоОбъект.Родитель = НайтиОбъектПоИдентификатору(структураОбъект.Родитель, Истина);
	...
	Если НадоДобавитьЗаписьВРегистр Тогда
		ЗаписатьИдентификаторВРегистр(ФизЛицоОбъект.Ссылка, структураОбъект.УИД);
	КонецЕсли;	
...
Процедура ЗагрузитьРезервыОценочныеОбязательства(структураОбъект, ОписаниеОшибок, ЗапрашиваемыеОбъекты)
	...
	ИначеЕсли НайденныеОбъекты = Неопределено Тогда
		...
		НадоДобавитьЗаписьВРегистр = Истина;
	Иначе
		РезервОбъект = НайденныеОбъекты[0].Ссылка.ПолучитьОбъект();	
	КонецЕсли;
	...
	Если НадоДобавитьЗаписьВРегистр Тогда
		ЗаписатьИдентификаторВРегистр(РезервОбъект.Ссылка, структураОбъект.УИД);
	КонецЕсли;	
...
Процедура ЗагрузитьЦенныеБумаги(структураОбъект, ОписаниеОшибок, ЗапрашиваемыеОбъекты)
	...
	ИначеЕсли НайденныеОбъекты = Неопределено Тогда
		...
		НадоДобавитьЗаписьВРегистр = Истина;
	Иначе
		ЦБОбъект = НайденныеОбъекты[0].Ссылка.ПолучитьОбъект();	
	КонецЕсли;
	...
	Если УИДЗаполнен(структураОбъект.Родитель) Тогда
		ЦБОбъект.Родитель = НайтиОбъектПоИдентификатору(структураОбъект.Родитель, Истина);
	...
	Если НадоДобавитьЗаписьВРегистр Тогда
		ЗаписатьИдентификаторВРегистр(ЦБОбъект.Ссылка, структураОбъект.УИД);
	КонецЕсли;	
...
Процедура ЗагрузитьОсновноеСредство(структураОбъект, ОписаниеОшибок, ЗапрашиваемыеОбъекты)
	...
	ИначеЕсли НайденныеОбъекты = Неопределено Тогда
		...
		НадоДобавитьЗаписьВРегистр = Истина;
	Иначе
		ОсновноеСредствоОбъект = НайденныеОбъекты[0].Ссылка.ПолучитьОбъект();	
	КонецЕсли;	
	...
	Если УИДЗаполнен(структураОбъект.Родитель) Тогда
		ОсновноеСредствоОбъект.Родитель = НайтиОбъектПоИдентификатору(структураОбъект.Родитель, Истина);
	...
	Если НадоДобавитьЗаписьВРегистр Тогда
		ЗаписатьИдентификаторВРегистр(ОсновноеСредствоОбъект.Ссылка, структураОбъект.УИД);
	КонецЕсли;	
...
Процедура ЗагрузитьОбъектСтроительства(структураОбъект, ОписаниеОшибок, ЗапрашиваемыеОбъекты)
	...
	ИначеЕсли НайденныеОбъекты = Неопределено Тогда
		...
		НадоДобавитьЗаписьВРегистр = Истина;
	Иначе
		ОбъектСтроительстваОбъект = НайденныеОбъекты[0].Ссылка.ПолучитьОбъект();	
	КонецЕсли;	
	...
	Если УИДЗаполнен(структураОбъект.Родитель) Тогда
		ОбъектСтроительстваОбъект.Родитель = НайтиОбъектПоИдентификатору(структураОбъект.Родитель, Истина);
	...
	Если НадоДобавитьЗаписьВРегистр Тогда
		ЗаписатьИдентификаторВРегистр(ОбъектСтроительстваОбъект.Ссылка, структураОбъект.УИД);
	КонецЕсли;	
...
Процедура ЗагрузитьНематериальныйАктив(структураОбъект, ОписаниеОшибок, ЗапрашиваемыеОбъекты)
	...
	ИначеЕсли НайденныеОбъекты = Неопределено Тогда
		...
		НадоДобавитьЗаписьВРегистр = Истина;
	Иначе
		НематериальныйАктивОбъект = НайденныеОбъекты[0].Ссылка.ПолучитьОбъект();	
	КонецЕсли;	
	...
	Если УИДЗаполнен(структураОбъект.Родитель) Тогда
		НематериальныйАктивОбъект.Родитель = НайтиОбъектПоИдентификатору(структураОбъект.Родитель, Истина);
	...
	Если НадоДобавитьЗаписьВРегистр Тогда
		ЗаписатьИдентификаторВРегистр(НематериальныйАктивОбъект.Ссылка, структураОбъект.УИД);
	КонецЕсли;	
...
Процедура ЗагрузитьСотрудника(структураОбъект, ОписаниеОшибок, ЗапрашиваемыеОбъекты)
	...
	ИначеЕсли НайденныеОбъекты = Неопределено Тогда
		...
		НадоДобавитьЗаписьВРегистр = Истина;
	Иначе
		СотрудникОбъект = НайденныеОбъекты[0].Ссылка.ПолучитьОбъект();	
	КонецЕсли;
	...
	СотрудникОбъект.ФизическоеЛицо = НайтиОбъектПоИдентификатору(структураОбъект.ФизическоеЛицо, Истина);
	...
	НайденныеОбъекты = НайтиОбъектПоИдентификатору(структураОбъект.ГоловнаяОрганизация);
	СотрудникОбъект.ГоловнаяОрганизация = НайтиОрганизацию(НайденныеОбъекты);
	...
	Если НадоДобавитьЗаписьВРегистр Тогда     
		СсылкаНового = Справочники.Сотрудники.ПолучитьСсылку();
		СотрудникОбъект.УстановитьСсылкуНового(СсылкаНового);
		СотрудникОбъект.ГоловнойСотрудник = СсылкаНового;
	Иначе
	...
	Если структураОбъект.Свойство("ГоловнойСотрудник") И УИДЗаполнен(структураОбъект.ГоловнойСотрудник) Тогда
		СотрудникОбъект.ГоловнойСотрудник = НайтиОбъектПоИдентификатору(структураОбъект.ГоловнойСотрудник, Истина);
	...
	Если НадоДобавитьЗаписьВРегистр Тогда
		ЗаписатьИдентификаторВРегистр(СотрудникОбъект.Ссылка, структураОбъект.УИД);
	КонецЕсли;	
...
Процедура ЗагрузитьСтатьюДДС(структураОбъект, ОписаниеОшибок, ЗапрашиваемыеОбъекты)
	...
	Если НайденныеОбъекты <> Неопределено И Не структураОбъект.ОбъектИзменен Тогда
		Возврат;
	ИначеЕсли НайденныеОбъекты = Неопределено Тогда
		Если ЗначениеЗаполнено(структураОбъект.ИмяПредопределенныхДанных) Тогда
		...
		НадоДобавитьЗаписьВРегистр = Истина;
	Иначе
		СтатьяДДСОбъект = НайденныеОбъекты[0].Ссылка.ПолучитьОбъект();	
	КонецЕсли;
	...
	Если УИДЗаполнен(структураОбъект.Родитель) Тогда
		СтатьяДДСОбъект.Родитель = НайтиОбъектПоИдентификатору(структураОбъект.Родитель, Истина);
...
Процедура ЗагрузитьБанк(структураОбъект, ОписаниеОшибок, ЗапрашиваемыеОбъекты)
	...
	ИначеЕсли НайденныеОбъекты = Неопределено Тогда
		...
		НадоДобавитьЗаписьВРегистр = Истина;
	Иначе
		БанкОбъект = НайденныеОбъекты[0].Ссылка.ПолучитьОбъект();	
	КонецЕсли;
	...
	Если УИДЗаполнен(структураОбъект.Родитель) Тогда
		БанкОбъект.Родитель = НайтиОбъектПоИдентификатору(структураОбъект.Родитель, Истина);
    ...
    Если Не структураОбъект.ЭтоГруппа Тогда
		...
		Если УИДЗаполнен(структураОбъект.Страна) Тогда
			БанкОбъект.Страна = НайтиОбъектПоИдентификатору(структураОбъект.Страна, Истина);
...
Процедура ЗагрузитьБанковскийСчет(структураОбъект, ОписаниеОшибок, ЗапрашиваемыеОбъекты)
	...
	Иначе   
		Для Каждого Стр Из НайденныеОбъекты Цикл
			Если (ТипЗнч(Стр.Ссылка.Владелец) = Тип("СправочникСсылка.Организации") И структураОбъект.ТипВладельца = "Организация") 
					ИЛИ (ТипЗнч(Стр.Ссылка.Владелец) = Тип("СправочникСсылка.Контрагенты") И структураОбъект.ТипВладельца = "Контрагент") Тогда
				БанковскийСчетОбъект = Стр.Ссылка.ПолучитьОбъект();
				БанковскийСчетНайден = Истина;
				Прервать;
			КонецЕсли;		
		КонецЦикла;	  
		Если Не БанковскийСчетНайден Тогда
			БанковскийСчетОбъект = Справочники.БанковскиеСчета.СоздатьЭлемент();				
			НадоДобавитьЗаписьВРегистр = Истина;			
		КонецЕсли;	
	КонецЕсли;	
	
	Если БанковскийСчетНайден И Не структураОбъект.ОбъектИзменен Тогда
		Возврат;
	КонецЕсли;		
	...
	Если УИДЗаполнен(структураОбъект.Владелец) Тогда
		НайденныеОбъекты = НайтиОбъектПоИдентификатору(структураОбъект.Владелец);
		Для Каждого Стр Из НайденныеОбъекты Цикл
			Если (ТипЗнч(Стр.Ссылка) = Тип("СправочникСсылка.Организации") И структураОбъект.ТипВладельца = "Организация") 
					ИЛИ (ТипЗнч(Стр.Ссылка) = Тип("СправочникСсылка.Контрагенты") И структураОбъект.ТипВладельца = "Контрагент") Тогда
				БанковскийСчетОбъект.Владелец = Стр.Ссылка;	
			КонецЕсли;		
		КонецЦикла;	
	КонецЕсли;	
	...
	Если УИДЗаполнен(структураОбъект.Банк) Тогда
		БанковскийСчетОбъект.Банк = НайтиОбъектПоИдентификатору(структураОбъект.Банк, Истина);
	...
	Если УИДЗаполнен(структураОбъект.ВалютаДенежныхСредств) Тогда
		БанковскийСчетОбъект.ВалютаДенежныхСредств = НайтиОбъектПоИдентификатору(структураОбъект.ВалютаДенежныхСредств, Истина);
	...
	Если УИДЗаполнен(структураОбъект.БанкДляРасчетов) Тогда
		БанковскийСчетОбъект.БанкДляРасчетов = НайтиОбъектПоИдентификатору(структураОбъект.БанкДляРасчетов, Истина);
	...
	Если НадоДобавитьЗаписьВРегистр Тогда
		ЗаписатьИдентификаторВРегистр(БанковскийСчетОбъект.Ссылка, структураОбъект.УИД);
	КонецЕсли;	
...
Процедура ЗагрузитьАвансовыйОтчет(структураАвансовыйОтчет, ОписаниеОшибок, ЗапрашиваемыеОбъекты)
	...
	//!! проверяем необходимые реквизиты (вообще надо ли это)?
	...
	//ОписаниеОшибок = "";
	//ПроверитьЗаполненностьОбязательныхПараметров(структураПоступление,структураОбязательныхПараметров,ОписаниеОшибок);
	//		
	...
	//новСтр.Дата = структураПоступление.Дата;
	...
	НадоДобавитьЗаписьВРегистр = Ложь;
  	АвансовыйОтчетСсылка = НайтиОбъектПоИдентификатору(структураАвансовыйОтчет.УИД, Истина);
...
//	АвансовыйОтчетОбъект.Склад = НайтиОбъектПоИдентификатору(структураАвансовыйОтчет.Склад, Истина);  //!!!
	...
	Попытка
		...
		Если НадоДобавитьЗаписьВРегистр Тогда
			ЗаписатьИдентификаторВРегистр(АвансовыйОтчетОбъект.Ссылка, структураАвансовыйОтчет.УИД);
		КонецЕсли;	
...
Процедура ЗагрузитьОбъектПоСтруктуре(структураОбъект, ОписаниеОшибок, ЗапрашиваемыеОбъекты)
	...
	Иначе
		...
		ОписаниеОшибок.errorDescriptionObj = "Для типа выгружаемого объекта " + структураОбъект.ТипОбъекта + " не задан алгоритм загрузки - " + структураОбъект.Представление;  //!!+ добавить представление объекта
	КонецЕсли;	
