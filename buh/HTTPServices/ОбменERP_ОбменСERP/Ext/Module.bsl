...
Процедура ЗагрузитьАвансовыйОтчет(структураДокумент, ОписаниеОшибок, ЗапрашиваемыеОбъекты)
	...
	ДокументОбъект.СуммаВключаетНДС = структураДокумент.ЦенаВключаетНДС;
	ДокументОбъект.ФизЛицо = НайтиОбъектПоИдентификаторуНов(структураДокумент.ПодотчетноеЛицо, Истина);
	...
	//!!
	Для Каждого Стр Из структураДокумент.Товары Цикл	
	...
	Попытка
		Если структураДокумент.Проведен Тогда
			Попытка
				структураДокумент.Записать(РежимЗаписиДокумента.Проведение);
			Исключение
				структураДокумент.Записать(РежимЗаписиДокумента.Запись); 
				//
				...
				ОписаниеОшибок.errorDescriptionObj = "Не удалось провести документ " + Строка(структураДокумент.Ссылка) + ". Документ был просто записан"; 
...
Процедура ЗагрузитьКорректировкуПоступления(структураДокумент, ОписаниеОшибок, ЗапрашиваемыеОбъекты)  
	...
	Иначе
		...
		ДокументОбъект.УстановитьСсылкуНового(Документы.КорректировкаПоступления.ПолучитьСсылку());	
		ЗаписатьИдентификаторВРегистр(ДокументОбъект.ПолучитьСсылкуНового(), структураДокумент.УИД);
	КонецЕсли; 
...
Процедура ЗагрузитьВозвратТоваровПоставщику(структураДокумент, ОписаниеОшибок, ЗапрашиваемыеОбъекты) 
	...
	Иначе
		...
		ДокументОбъект.УстановитьСсылкуНового(Документы.ВозвратТоваровПоставщику.ПолучитьСсылку());	
		ЗаписатьИдентификаторВРегистр(ДокументОбъект.ПолучитьСсылкуНового(), структураДокумент.УИД);
	КонецЕсли;                                    
...
Процедура ЗагрузитьОплатуПлатежнойКартой(структураДокумент, ОписаниеОшибок, ЗапрашиваемыеОбъекты)
	...
	Иначе
		...
		ДокументОбъект.УстановитьСсылкуНового(Документы.ОплатаПлатежнойКартой.ПолучитьСсылку());	
		ЗаписатьИдентификаторВРегистр(ДокументОбъект.ПолучитьСсылкуНового(), структураДокумент.УИД);
	КонецЕсли;	
	...
	Для Каждого структураСтрока Из структураДокумент.РасшифровкаПлатежа Цикл	
		...
		//ДОДЕЛАТЬ!!!
		//ЗаполнитьРеквизитПоДаннымERP(Нстр, "Сделка", структураДокумент, "Сделка", , ЗапрашиваемыеОбъекты);
		...
		Если ЗначениеЗаполнено(структураСтрока.СчетУчетаРасчетовСКонтрагентом) Тогда
		...
		КонецЕсли;
		Если ЗначениеЗаполнено(структураСтрока.СчетУчетаРасчетовПоАвансам) Тогда
