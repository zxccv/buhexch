...
Процедура ЗагрузитьПроводкиДокумента(ДокументОбъект, структураПроводки, ЗапрашиваемыеОбъекты)
	...
	Для Каждого структураПроводка Из структураПроводки Цикл
		...
		новДвижение.СчетДт = ПланыСчетов.Хозрасчетный.НайтиПоРеквизиту("КодБыстрогоВыбора", структураПроводка.СчетДт);
		...
		новДвижение.СчетКт = ПланыСчетов.Хозрасчетный.НайтиПоРеквизиту("КодБыстрогоВыбора", структураПроводка.СчетКт);
...
Процедура ЗагрузитьАвансовыйОтчет(структураДокумент, ОписаниеОшибок, ЗапрашиваемыеОбъекты)
	...
	//АвансовыйОтчетОбъект.Ответственный = НайтиОбъектПоИдентификаторуНов(структураДокумент.Автор, Истина);
	...
	//Самойлов. Погасил пока следующие два цикла, пока грузим АО только из поступлений
	...
	Для Каждого структураСтрока Из структураДокумент.Прочее Цикл
		...
		Если структураСтрока.РаспределитьНаСебестоимость Тогда
...
Процедура ЗагрузитьКорректировкуПоступления(структураДокумент, ОписаниеОшибок, ЗапрашиваемыеОбъекты)  
	...
	ЗаполнитьРеквизитПоДаннымERP(ДокументОбъект, "Организация", структураДокумент, "Организация", Тип("СправочникСсылка.Организации"), ЗапрашиваемыеОбъекты);
	ЗаполнитьРеквизитПоДаннымERP(ДокументОбъект, "Склад", структураДокумент, "Склад", Тип("СправочникСсылка.Склады"), ЗапрашиваемыеОбъекты);  
	ЗаполнитьРеквизитПоДаннымERP(ДокументОбъект, "ДокументПоступления", структураДокумент, "ДокументПоступления", , ЗапрашиваемыеОбъекты, Документы.ПоступлениеТоваровУслуг.ПустаяСсылка());   
	
	Если ТипЗнч(ДокументОбъект.ДокументПоступления) = Тип("ДокументСсылка.ДокументРасчетовСКонтрагентом") Тогда
		ДокументОбъект.КорректироватьБУиНУ = Ложь;
	Иначе
		ДокументОбъект.КорректироватьБУиНУ = Истина;
	КонецЕсли;
	...
	Для Каждого структураСтрокаТовар Из структураДокумент.Товары Цикл
		...
		Если структураСтрокаТовар.ЭтоУслуга Тогда
			...
			ЗаполнитьРеквизитПоДаннымERP(новСтрТовар, "Номенклатура", структураСтрокаТовар, "Номенклатура", , ЗапрашиваемыеОбъекты, Справочники.Номенклатура.ПустаяСсылка());
		Иначе
			новСтрТовар = ДокументОбъект.Товары.Добавить(); 
			...
			КонецЕсли;
 			Если ЗначениеЗаполнено(структураСтрокаТовар.СчетРасчетов) Тогда
				ЗаполнитьРеквизитПоДаннымERP(новСтрТовар, "СчетУчетаНДС", структураСтрокаТовар, "СчетУчетаНДС", , ЗапрашиваемыеОбъекты);
			КонецЕсли;
			Если ЗначениеЗаполнено(структураСтрокаТовар.СчетРасчетов) Тогда
				ЗаполнитьРеквизитПоДаннымERP(новСтрТовар, "СчетРасчетов", структураСтрокаТовар, "СчетРасчетов", , ЗапрашиваемыеОбъекты);
			КонецЕсли;
...
Процедура ЗагрузитьКорректировкуДолга(структураДокумент, ОписаниеОшибок, ЗапрашиваемыеОбъекты)
	...
	ВидОперации = структураДокумент.ВидОперации;
	Если ВидОперации = "ПереносДолгаПоставщикуОрганизацияКонтрагент" Тогда //Перенос долга поставщику в рамках организации и контрагента
		ДокументОбъект.ВидОперации = Перечисления.ВидыОперацийКорректировкаДолга.ПереносЗадолженности; 
	ИначеЕсли ВидОперации = "ПоставщикаМеждуОрганизациями" Тогда //Взаимозачет с участием двух организаций для одного поставщика
	ИначеЕсли ВидОперации = "ПереносДолгаПоставщикуМеждуКонтрагентами" //Перенос долга поставщику между контрагентами
		ИЛИ ВидОперации = "ПереносДолгаКлиентаОрганизацияКонтрагент" Тогда //Перенос долга клиента в рамках организации и контрагента 
		ДокументОбъект.ВидОперации = Перечисления.ВидыОперацийКорректировкаДолга.ПереносЗадолженности; 
	ИначеЕсли ВидОперации = "ПереносАвансаКлиентаМеждуКонтрагентами" //Перенос аванса клиента между двумя контрагентами 
		ИЛИ ВидОперации = "ПереносАвансаПоставщикуОрганизацияКонтрагент" //Перенос аванса поставщику в рамках организации и контрагента 
		ИЛИ ВидОперации = "ПереносАвансаКлиентаОрганизацияКонтрагент" Тогда //Перенос аванса клиента в рамках организации и контрагента 
		ДокументОбъект.ВидОперации = Перечисления.ВидыОперацийКорректировкаДолга.ЗачетАвансов; 
	ИначеЕсли ВидОперации = "Произвольный" //Произвольный взаимозачет
		ИЛИ ВидОперации = "Бартер" Тогда //Зачет встречных обязательств
	...
	Для Каждого структураСтрока Из структураДокумент.ДебиторскаяЗадолженность Цикл	
		...
		ЗаполнитьРеквизитПоДаннымERP(НоваяСтрока, "Сделка", структураСтрока, "ОбъектРасчетов", , ЗапрашиваемыеОбъекты); 
	...
	Для Каждого структураСтрока Из структураДокумент.КредиторскаяЗадолженность Цикл	
		...
		ПараметрыДоговора.Вставить("ВидДоговора", "СПоставщиком"); 
		ПараметрыДоговора.Вставить("Валюта", ДокументОбъект.ВалютаДокумента);  
		НоваяСтрока.ДоговорКонтрагента = НайтиСоздатьОсновнойДоговорКонтрагента(ДокументОбъект.Организация, ДокументОбъект.КонтрагентКредитор, ПараметрыДоговора);
...
Процедура ЗагрузитьРеализацию(структураДокумент, ОписаниеОшибок, ЗапрашиваемыеОбъекты)
	...
	Для Каждого структураСтрокаТовар Из структураДокумент.Товары Цикл
		...
		Если НЕ ЗначениеЗаполнено(СкладДокумента)
			И ЗначениеЗаполнено(структураСтрокаТовар.Склад) Тогда
			СкладДокумента = структураСтрокаТовар.Склад;
		КонецЕсли;                                      
		
		Если ЗначениеЗаполнено(СкладДокумента)
			И ЗначениеЗаполнено(структураСтрокаТовар.Склад)
		...
		Иначе
			...
			Если ЗначениеЗаполнено(структураСтрокаТовар.СчетУчета) Тогда
				ЗаполнитьРеквизитПоДаннымERP(новСтрУслуга, "СчетДоходов", структураСтрокаТовар, "СчетУчета", , ЗапрашиваемыеОбъекты);
			КонецЕсли;
			
			Если ЗначениеЗаполнено(структураСтрокаТовар.СчетУчетаНДС) Тогда
				ЗаполнитьРеквизитПоДаннымERP(новСтрУслуга,"СчетУчетаНДСПоРеализации",структураСтрокаТовар,"СчетУчетаНДС",,ЗапрашиваемыеОбъекты);
			КонецЕсли;         
...
Процедура СформироватьПеремещенияНаСкладОтгузки(ДокументРеализация,УИДРеализации,мсвСтрокиДляПеремещения)
	...
	Для Каждого СкладИзУТ Из мсвСкладыДляПеремещений Цикл
		...
		Если ИспользованоСуществующихПеремещений < мсвСуществующиеПеремещения.Количество() Тогда
			ПеремещениеОбъект = мсвСуществующиеПеремещения[ИспользованоСуществующихПеремещений];
...
Процедура ЗагрузитьТребованиеНакладную(структураДокумент, ОписаниеОшибок, ЗапрашиваемыеОбъекты)
	...
	Если ЗначениеЗаполнено(структураДокумент.Префикс) Тогда
...
Процедура ЗагрузитьОбъектПоСтруктуре(структураОбъект, ОписаниеОшибок, ЗапрашиваемыеОбъекты)
	...
	ИначеЕсли структураОбъект.ТипОбъекта = "ВозвратТоваровПоставщикуКакКорректировкаПриобретения" 
		ИЛИ структураОбъект.ТипОбъекта = "КорректировкаПриобретенияКакКорректировкаПриобретения" Тогда	
		ЗагрузитьКорректировкуПоступления(структураОбъект, ОписаниеОшибок, ЗапрашиваемыеОбъекты);
	ИначеЕсли структураОбъект.ТипОбъекта = "ВзаимозачетЗадолженности" Тогда
