...
Процедура ВыгрузитьВзаимозачетЗадолженности(СсылкаНаОбъект, ТаблицаВыгрузки, ОписаниеОшибок)  
	...
	Для каждого СтрДЗ Из ТаблицаДебиторскаяЗадолженность Цикл  
		...
		НСтр = Новый Структура;
		НСтр.Вставить("ОбъектРасчетов", СформироватьУИДДляВыгрузки(ДокументРасчетов));
		...
		Структура = Новый Структура("Склад");
		ЗаполнитьЗначенияСвойств(Структура, ДокументРасчетов);
		СкладСчета = Справочники.СтруктураПредприятия.ПустаяСсылка();
		Если ЗначениеЗаполнено(Структура.Склад) Тогда 
			СкладСчета = Структура.Склад;
		КонецЕсли;   
		
		НСтр.Вставить("СчетУчетаРасчетов", КодСчетаРегл(ПолучитьСчетУчетаИзНастроек(СтруктураВидыСчетов.СчетДебитораРасчеты, СсылкаНаОбъект.Организация, ДокументРасчетов.ГруппаФинансовогоУчета, СкладСчета)));
		НСтр.Вставить("СчетУчетаАвансов", КодСчетаРегл(ПолучитьСчетУчетаИзНастроек(СтруктураВидыСчетов.СчетДебитораАвансы, СсылкаНаОбъект.Организация, ДокументРасчетов.ГруппаФинансовогоУчета, СкладСчета)));
	...
	Для каждого СтрКЗ Из ТаблицаКредиторскаяЗадолженность Цикл  
		...
		НСтр.Вставить("СчетУчетаРасчетов", КодСчетаРегл(ПолучитьСчетУчетаИзНастроек(СтруктураВидыСчетов.СчетКредитораРасчеты, СсылкаНаОбъект.Организация, ДокументРасчетов.ГруппаФинансовогоУчета, СкладСчета)));
		НСтр.Вставить("СчетУчетаАвансов", КодСчетаРегл(ПолучитьСчетУчетаИзНастроек(СтруктураВидыСчетов.СчетКредитораАвансы, СсылкаНаОбъект.Организация, ДокументРасчетов.ГруппаФинансовогоУчета, СкладСчета)));
