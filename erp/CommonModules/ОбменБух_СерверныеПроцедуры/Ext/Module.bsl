...
Процедура ЗарегистрироватьИзменения(Объект, Узел = Неопределено) Экспорт  
	...
	Если ТипЗнч(Объект.Ссылка) = Тип("ДокументСсылка.ПриобретениеТоваровУслуг") 
			И Объект.ХозаяйственнаяОперация = Перечисления.ХозяйственныеОперации.ЗакупкаЧерезПодотчетноеЛицо Тогда
...
Процедура ВыгрузитьПриобретение(СсылкаНаОбъект, ОбъектИзменен, ТаблицаВыгрузки, ОписаниеОшибок)
	...
	//СтруктураВыгрузки = Новый Структура;
	//СтруктураВыгрузки.Вставить("УИД", Строка(СсылкаНаОбъект.УникальныйИдентификатор())); 
	//СтруктураВыгрузки.Вставить("ОбъектИзменен", ОбъектИзменен);	
	//СтруктураВыгрузки.Вставить("ТипОбъекта", "Банк");		
	//СтруктураВыгрузки.Вставить("Код", СсылкаНаОбъект.Код); 
	//СтруктураВыгрузки.Вставить("Наименование", СсылкаНаОбъект.Наименование); 
	//СтруктураВыгрузки.Вставить("ПометкаУдаления", СсылкаНаОбъект.ПометкаУдаления);	
	//СтруктураВыгрузки.Вставить("Родитель", Строка(СсылкаНаОбъект.Родитель.УникальныйИдентификатор()));
	//ВыгрузитьБанк(СсылкаНаОбъект.Родитель, Ложь, ТаблицаВыгрузки, ОписаниеОшибок);	
	//СтруктураВыгрузки.Вставить("ЭтоГруппа", СсылкаНаОбъект.ЭтоГруппа);
	//Если Не СсылкаНаОбъект.ЭтоГруппа Тогда
	//	СтруктураВыгрузки.Вставить("КоррСчет", СсылкаНаОбъект.КоррСчет);
	...
	//	СтруктураВыгрузки.Вставить("КодАльфа3", СсылкаНаОбъект.КодАльфа3);
	//	СтруктураВыгрузки.Вставить("СВИФТБИК", СсылкаНаОбъект.СВИФТБИК);
	//	СтруктураВыгрузки.Вставить("Страна", Строка(СсылкаНаОбъект.Страна.УникальныйИдентификатор()));
	//	ВыгрузитьСтрану(СсылкаНаОбъект.Страна, Ложь, ТаблицаВыгрузки, ОписаниеОшибок);
	//	
	//	Если СсылкаНаОбъект.ДеятельностьПрекращена Тогда
	//		СтруктураВыгрузки.Вставить("РучноеИзменение", 3);
...
Функция ВыгрузитьОбъект(СсылкаНаОбъект = Неопределено, ОбъектИзменен = Истина, ТаблицаВыгрузки = Неопределено, ОписаниеОшибок = Неопределено)
	...
	ИначеЕсли ТипОбъекта = Тип("ДокументСсылка.ОперацияПоПлатежноКарте") Тогда
