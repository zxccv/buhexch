...
Процедура ЗарегистрироватьИзменения(Объект, Узел = Неопределено) Экспорт  
	...
	Если ТипЗнч(Объект.Ссылка) = Тип("ДокументСсылка.ВзаимозачетЗадолженности") Тогда  
		Если НЕ (Объект.КонтрагентДебитор <> Объект.КонтрагентКредитор
			ИЛИ Объект.ТипДебитора <> Объект.ТипКредитора) Тогда
			Возврат;
		КонецЕсли;
	КонецЕсли;
...
Функция СформироватьУИДДляВыгрузки(Ссылка)
	...
	Если Ссылка = Неопределено Тогда
		Возврат Строка(Новый УникальныйИдентификатор("00000000-0000-0000-0000-000000000000")); 
	КонецЕсли;
...
Функция ВыгрузитьОбъект(СсылкаНаОбъект = Неопределено, ТаблицаВыгрузки = Неопределено, ОписаниеОшибок = Неопределено)
	...
	ИначеЕсли ТипОбъекта = Тип("ДокументСсылка.КорректировкаЗадолженности") Тогда
		ВыгрузитьКорректировкуЗадолженности(СсылкаНаОбъект, ТаблицаВыгрузки, ОписаниеОшибок);
	...
	ИначеЕсли ЭтотДокументВыгружаетсяКакОперацияБух(ТипОбъекта)	Тогда
	...
	ИначеЕсли ТипОбъекта = Тип("ДокументСсылка.РаспределениеРасходовБудущихПериодов") Тогда
		//Надо потом сюда метод написать, пока они все выгружены разом.
	Иначе
...
Процедура ВыгрузитьДопОбъект(СтруктураОбъекта, ДопТаблицаВыгрузки, ОписаниеОшибок)
	...
	ИначеЕсли (Метаданные.Документы.Содержит(Метаданные.НайтиПоТипу(ТипОбъекта)) И ОбъектСсылка.Дата < Дата(2021,01,01,00,00,00) И НЕ ТипЗнч(ОбъектСсылка) = Тип("ДокументСсылка.ВедомостьНаВыплатуЗарплатыВБанк"))
	...
	ИначеЕсли ТипОбъекта = Тип("СправочникСсылка.КлассификаторБанков") Тогда
		...
		КонецЕсли; 
	ИначеЕсли ТипОбъекта = Тип("СправочникСсылка.ОбъектыЭксплуатации") Тогда
		Если СтрНайти(СтруктураОбъекта.ТипПриемника, "ОбъектСтроительства") > 0 Тогда
			ВыгрузитьОбъектСтроительства(ОбъектСсылка, ДопТаблицаВыгрузки, ОписаниеОшибок);
		Иначе
			ВыгрузитьОбъектЭксплуатации(ОбъектСсылка, ДопТаблицаВыгрузки, ОписаниеОшибок);
		КонецЕсли;
	Иначе     
...
Процедура ОтправитьПакет(Узел, ТаблицаВыгрузки, НомерСообщения)
	...
	МаксПопыток = 7; //!! 5 должно быть задано в реквизите узла обмена КоличествоПовторныхЗапросов
	...
	Иначе   
		//Савина Н. #1 Начало 29.09.2022
		Если текНомерПопытки > МаксПопыток И РезультатВыгрузки.requestedObj.Количество() > 0 Тогда
...
Функция ЭтотДокументВыгружаетсяКакОперацияБух(ТипОбъекта) Экспорт 
	
	Если ТипОбъекта = Тип("ДокументСсылка.ОперацияБух")
		...
		ИЛИ ТипОбъекта = Тип("ДокументСсылка.НачислениеСписаниеРезервовПоСомнительнымДолгам")
		ИЛИ ТипОбъекта = Тип("ДокументСсылка.НачислениеОценочныхОбязательствПоОтпускам") Тогда
		Возврат Истина;
	КонецЕсли;         
...
Процедура ВыгрузитьВзаимозачетЗадолженности(СсылкаНаОбъект, ТаблицаВыгрузки, ОписаниеОшибок)  
	...
	Если НЕ (СсылкаНаОбъект.КонтрагентДебитор <> СсылкаНаОбъект.КонтрагентКредитор
		ИЛИ СсылкаНаОбъект.ТипДебитора <> СсылкаНаОбъект.ТипКредитора) Тогда
		Возврат;
	КонецЕсли;
	...
	СтруктураВыгрузки.Вставить("ТипДебитора", XMLСтрока(СсылкаНаОбъект.ТипДебитора));
	СтруктураВыгрузки.Вставить("ТипКредитора", XMLСтрока(СсылкаНаОбъект.ТипКредитора));
	СтруктураВыгрузки.Вставить("Комментарий", СсылкаНаОбъект.Комментарий);
	СтруктураВыгрузки.Вставить("ВидОперации", XMLСтрока(СсылкаНаОбъект.ВидОперации)); 	
	//                         
	ЗапросСверткиТаблицы = Новый Запрос;
	ЗапросСверткиТаблицы.Текст = " 
	|ВЫБРАТЬ
	|	ТаблицаДокументаДебиторскаяЗадолженность.ОбъектРасчетов КАК ОбъектРасчетов,
	|	ТаблицаДокументаДебиторскаяЗадолженность.СуммаВзаиморасчетов КАК СуммаВзаиморасчетов
	|ПОМЕСТИТЬ ВТ_ТаблицаДокументаДебиторскаяЗадолженность
	|ИЗ
	|	&ТаблицаДокументаДебиторскаяЗадолженность КАК ТаблицаДокументаДебиторскаяЗадолженность
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	МАКСИМУМ(ВТ_ДЗ.ОбъектРасчетов) КАК ОбъектРасчетов,
	|	СУММА(ВТ_ДЗ.СуммаВзаиморасчетов) КАК СуммаВзаиморасчетов
	|ПОМЕСТИТЬ ВТ_ТаблицаДебиторскаяЗадолженность
	|ИЗ
	|	ВТ_ТаблицаДокументаДебиторскаяЗадолженность КАК ВТ_ДЗ
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ТаблицаДокументаКредиторскаяЗадолженность.ОбъектРасчетов КАК ОбъектРасчетов,
	|	ТаблицаДокументаКредиторскаяЗадолженность.СуммаВзаиморасчетов КАК СуммаВзаиморасчетов
	|ПОМЕСТИТЬ ВТ_ТаблицаДокументаКредиторскаяЗадолженность
	|ИЗ
	|	&ТаблицаДокументаКредиторскаяЗадолженность КАК ТаблицаДокументаКредиторскаяЗадолженность
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	МАКСИМУМ(ВТ_КЗ.ОбъектРасчетов) КАК ОбъектРасчетов,
	|	СУММА(ВТ_КЗ.СуммаВзаиморасчетов) КАК СуммаВзаиморасчетов
	|ПОМЕСТИТЬ ВТ_ТаблицаКредиторскаяЗадолженность
	|ИЗ
	|	ВТ_ТаблицаДокументаКредиторскаяЗадолженность КАК ВТ_КЗ
	|";
	
	ЗапросСверткиТаблицы.УстановитьПараметр("ТаблицаДокументаДебиторскаяЗадолженность", СсылкаНаОбъект.ДебиторскаяЗадолженность.Выгрузить(, "ОбъектРасчетов, СуммаВзаиморасчетов"));
	ЗапросСверткиТаблицы.УстановитьПараметр("ТаблицаДокументаКредиторскаяЗадолженность", СсылкаНаОбъект.ДебиторскаяЗадолженность.Выгрузить(, "ОбъектРасчетов, СуммаВзаиморасчетов"));
	
	РезультатПакета = ЗапросСверткиТаблицы.ВыполнитьПакетСПромежуточнымиДанными();
	ТаблицаДебиторскаяЗадолженность = РезультатПакета[1].Выгрузить();
	ТаблицаКредиторскаяЗадолженность = РезультатПакета[3].Выгрузить();
	
	СтруктураВидыСчетов = Новый Структура();
	СтруктураВидыСчетов.Вставить("СчетДебитораРасчеты", ?(СтруктураВыгрузки.ТипДебитора = "Поставщик", Перечисления.ВидыСчетовРеглУчета.РасчетыСПоставщиками, Перечисления.ВидыСчетовРеглУчета.РасчетыСКлиентами));
	СтруктураВидыСчетов.Вставить("СчетДебитораАвансы", ?(СтруктураВыгрузки.ТипДебитора = "Поставщик", Перечисления.ВидыСчетовРеглУчета.АвансыВыданные, Перечисления.ВидыСчетовРеглУчета.АвансыПолученные));
	СтруктураВидыСчетов.Вставить("СчетКредитораРасчеты", ?(СтруктураВыгрузки.ТипКредитора = "Поставщик", Перечисления.ВидыСчетовРеглУчета.РасчетыСПоставщиками, Перечисления.ВидыСчетовРеглУчета.РасчетыСКлиентами));
	СтруктураВидыСчетов.Вставить("СчетКредитораАвансы", ?(СтруктураВыгрузки.ТипКредитора = "Поставщик", Перечисления.ВидыСчетовРеглУчета.АвансыВыданные, Перечисления.ВидыСчетовРеглУчета.АвансыПолученные));
	
	ДебиторскаяЗадолженность = Новый Массив;  
	Для каждого СтрДЗ Из ТаблицаДебиторскаяЗадолженность Цикл  
		ДокументРасчетов = СтрДЗ.ОбъектРасчетов.Объект;  
		
		НСтр = Новый Структура;
		НСтр.Вставить("СуммаВзаиморасчетов", СтрДЗ.СуммаВзаиморасчетов);  
		
		Если НЕ ЗначениеЗаполнено(СсылкаНаОбъект.Подразделение) Тогда
			СтруктураВыгрузки.Вставить("Подразделение", СформироватьУИДДляВыгрузки(ДокументРасчетов.Подразделение));
		КонецЕсли;
		
		Структура = Новый Структура("Склад", Справочники.СтруктураПредприятия.ПустаяСсылка());
		ЗаполнитьЗначенияСвойств(Структура, ДокументРасчетов);
		
		НСтр.Вставить("СчетУчетаРасчетов", КодСчетаРегл(ПолучитьСчетУчетаИзНастроек(СтруктураВидыСчетов.СчетДебитораРасчеты, СсылкаНаОбъект.Организация, ДокументРасчетов.ГруппаФинансовогоУчета, Структура.Склад)));
		НСтр.Вставить("СчетУчетаАвансов", КодСчетаРегл(ПолучитьСчетУчетаИзНастроек(СтруктураВидыСчетов.СчетДебитораАвансы, СсылкаНаОбъект.Организация, ДокументРасчетов.ГруппаФинансовогоУчета, Структура.Склад)));
	...
	Для каждого СтрКЗ Из ТаблицаКредиторскаяЗадолженность Цикл  
		ДокументРасчетов = СтрКЗ.ОбъектРасчетов.Объект;  
		
		НСтр = Новый Структура;
		НСтр.Вставить("СуммаВзаиморасчетов", СтрКЗ.СуммаВзаиморасчетов);
		
		Структура = Новый Структура("Склад", Справочники.СтруктураПредприятия.ПустаяСсылка());
		ЗаполнитьЗначенияСвойств(Структура, ДокументРасчетов);
		
		НСтр.Вставить("СчетУчетаРасчетов", КодСчетаРегл(ПолучитьСчетУчетаИзНастроек(СтруктураВидыСчетов.СчетКредитораРасчеты, СсылкаНаОбъект.Организация, ДокументРасчетов.ГруппаФинансовогоУчета, Структура.Склад)));
		НСтр.Вставить("СчетУчетаАвансов", КодСчетаРегл(ПолучитьСчетУчетаИзНастроек(СтруктураВидыСчетов.СчетКредитораАвансы, СсылкаНаОбъект.Организация, ДокументРасчетов.ГруппаФинансовогоУчета, Структура.Склад)));
...
Процедура ВыгрузитьПоступлениеБезналичныхДС(СсылкаНаОбъект, ТаблицаВыгрузки, ОписаниеОшибок)
	...
	СтруктураВыгрузки.Вставить("Номер", СсылкаНаОбъект.Номер); 
	Если ЗначениеЗаполнено(СсылкаНаОбъект.ДатаПроведенияБанком) Тогда
		СтруктураВыгрузки.Вставить("Дата", СсылкаНаОбъект.ДатаПроведенияБанком);
	Иначе
		СтруктураВыгрузки.Вставить("Дата", СсылкаНаОбъект.Дата);
	КонецЕсли;
	СтруктураВыгрузки.Вставить("Проведен", СсылкаНаОбъект.Проведен);	
...
Процедура ВыгрузитьСписаниеБезналичныхДенежныхСредств(СсылкаНаОбъект, ТаблицаВыгрузки, ОписаниеОшибок)
	...
	Если ЗначениеЗаполнено(СсылкаНаОбъект.НастройкаСчетовУчета) И ЗначениеЗаполнено(СсылкаНаОбъект.НастройкаСчетовУчета.СчетУчета) Тогда
		...
		СтруктураВыгрузки.Вставить("Субконто1", "");
		СтруктураВыгрузки.Вставить("ВидСубконто1", "");
		СтруктураВыгрузки.Вставить("Субконто2", "");
		СтруктураВыгрузки.Вставить("ВидСубконто2", "");
		
		Если ЗначениеЗаполнено(СсылкаНаОбъект.НастройкаСчетовУчета.Субконто1) Тогда			
			СтруктураВыгрузки.ВидСубконто1 = СсылкаНаОбъект.НастройкаСчетовУчета.Субконто1.Метаданные().ПолноеИмя();			
			Если ОбщегоНазначения.ЭтоПеречисление(СсылкаНаОбъект.НастройкаСчетовУчета.Субконто1.Метаданные()) Тогда
				СтруктураВыгрузки.Субконто1 = XMLСтрока(СсылкаНаОбъект.НастройкаСчетовУчета.Субконто1);
			Иначе
				СтруктураВыгрузки.Субконто1 = СформироватьУИДДляВыгрузки(СсылкаНаОбъект.НастройкаСчетовУчета.Субконто1);				
			КонецЕсли; 			
		КонецЕсли;
		Если ЗначениеЗаполнено(СсылкаНаОбъект.НастройкаСчетовУчета.Субконто2) Тогда			
			СтруктураВыгрузки.ВидСубконто2 = СсылкаНаОбъект.НастройкаСчетовУчета.Субконто2.Метаданные().ПолноеИмя();			
			Если ОбщегоНазначения.ЭтоПеречисление(СсылкаНаОбъект.НастройкаСчетовУчета.Субконто2.Метаданные()) Тогда
				СтруктураВыгрузки.Субконто2 = XMLСтрока(СсылкаНаОбъект.НастройкаСчетовУчета.Субконто2);
			Иначе
				СтруктураВыгрузки.Субконто2 = СформироватьУИДДляВыгрузки(СсылкаНаОбъект.НастройкаСчетовУчета.Субконто2);				
			КонецЕсли; 			
		КонецЕсли;	
	...
	Для каждого Стр Из СсылкаНаОбъект.РасшифровкаПлатежа Цикл
		...
		МассивТиповОбъектов.Добавить(Тип("ДокументСсылка.ПриобретениеТоваровУслуг"));
		МассивТиповОбъектов.Добавить(Тип("ДокументСсылка.ЗаказПоставщику"));
		...
		ИначеЕсли ЗначениеЗаполнено(Стр.НастройкаСчетовУчета) И ЗначениеЗаполнено(Стр.НастройкаСчетовУчета.СчетУчета) Тогда 
			СтруктураВыгрузки.Вставить("СчетУчетаРасчетовСКонтрагентом", КодСчетаРегл(Стр.НастройкаСчетовУчета.СчетУчета)); 
		ИначеЕсли ЗначениеЗаполнено(ОбъектРасчетов)	И МассивТиповОбъектов.Найти(ТипЗнч(ОбъектРасчетов)) <> Неопределено Тогда
...
Процедура ВыгрузитьПередачуТоваровМеждуОрганизациями(СсылкаНаОбъект, ТаблицаВыгрузки, ОписаниеОшибок)
	...
	СтруктураВыгрузки.Вставить("УИД", СформироватьУИДДляВыгрузки(СсылкаНаОбъект)); 	
	СтруктураВыгрузки.Вставить("ТипОбъекта", "ПриобретениеТоваровУслуг");  
	СтруктураВыгрузки.Вставить("Префикс", "ПТ"); 
	СтруктураВыгрузки.Вставить("Номер", СсылкаНаОбъект.Номер); 
...
Процедура ВыгрузитьДокументКакДокументРасчетовСКонтрагентами(СсылкаНаОбъект, ТаблицаВыгрузки, ОписаниеОшибок) 
	...
	ИначеЕсли ТипЗнч(СсылкаНаОбъект) = Тип("ДокументСсылка.КорректировкаРеализации") Тогда
		СтруктураВыгрузки.Вставить("Префикс","КР");
		СтруктураВыгрузки.Вставить("ВидДоговора", "СПокупателем");		
	ИначеЕсли ТипЗнч(СсылкаНаОбъект) = Тип("ДокументСсылка.ПоступлениеБезналичныхДенежныхСредств") Тогда
		СтруктураВыгрузки.Вставить("Префикс","ПБ");
		СтруктураВыгрузки.Вставить("ВидДоговора", "СПокупателем");				
	Иначе
	...
	Если ТипЗнч(СсылкаНаОбъект) = Тип("ДокументСсылка.ОтчетКомиссионера") Тогда
		СтруктураВыгрузки.Вставить("Префикс","ОК");
	КонецЕсли;
...
Процедура ВыгрузитьКорректировкуЗадолженности(СсылкаНаОбъект, ТаблицаВыгрузки, ОписаниеОшибок)
	...
	СтруктураВыгрузки.Вставить("УИД", СформироватьУИДДляВыгрузки(СсылкаНаОбъект)); 
	СтруктураВыгрузки.Вставить("Представление", Строка(СсылкаНаОбъект));
	СтруктураВыгрузки.Вставить("ТипОбъекта", "КорректировкаЗадолженности");
	СтруктураВыгрузки.Вставить("Номер", СсылкаНаОбъект.Номер); 
	СтруктураВыгрузки.Вставить("Дата", СсылкаНаОбъект.Дата);
	СтруктураВыгрузки.Вставить("Проведен", СсылкаНаОбъект.Проведен);	
	СтруктураВыгрузки.Вставить("ПометкаУдаления", СсылкаНаОбъект.ПометкаУдаления); 
	...
	Для каждого Стр Из СсылкаНаОбъект.Задолженность Цикл
		НСтр = Новый Структура;            
		
		ГФУ = Справочники.ГруппыФинансовогоУчетаРасчетов.ПустаяСсылка();
						
		Если Стр.ТипРасчетов = Перечисления.ТипыРасчетовСПартнерами.РасчетыСПоставщиком Тогда
			НСтр.Вставить("СчетУчетаРасчетов", КодСчетаРегл(ПолучитьСчетУчетаИзНастроек(Перечисления.ВидыСчетовРеглУчета.РасчетыСПоставщиками, СсылкаНаОбъект.Организация, ГФУ, )));
			НСтр.Вставить("СчетУчетаАвансов", КодСчетаРегл(ПолучитьСчетУчетаИзНастроек(Перечисления.ВидыСчетовРеглУчета.АвансыВыданные, СсылкаНаОбъект.Организация, ГФУ, )));
		Иначе
			НСтр.Вставить("СчетУчетаРасчетов", КодСчетаРегл(ПолучитьСчетУчетаИзНастроек(Перечисления.ВидыСчетовРеглУчета.РасчетыСКлиентами, СсылкаНаОбъект.Организация, ГФУ, )));
			НСтр.Вставить("СчетУчетаАвансов", КодСчетаРегл(ПолучитьСчетУчетаИзНастроек(Перечисления.ВидыСчетовРеглУчета.АвансыПолученные, СсылкаНаОбъект.Организация, ГФУ, )));
		КонецЕсли;   
		
		НСтр.Вставить("ТипРасчетов", XMLстрока(Стр.ТипРасчетов)); 
		НСтр.Вставить("Сумма", Стр.Сумма); 
				
		Задолженность.Добавить(НСтр);
	КонецЦикла;
	СтруктураВыгрузки.Вставить("Задолженность", Задолженность); 
	
	ДоходыРасходыАктивыПассивы = Новый Массив; 
	Для каждого Стр Из СсылкаНаОбъект.ДоходыРасходыАктивыПассивы Цикл 
			
		Если ЗначениеЗаполнено(Стр.Статья) Тогда
			Если НЕ ТипЗнч(Стр.Статья) = Тип("ПланВидовХарактеристикСсылка.СтатьиАктивовПассивов") И ЗначениеЗаполнено(Стр.Статья.ГруппаФинансовогоУчета) Тогда			
				Запрос = Новый Запрос;
				Запрос.Текст = 
				"ВЫБРАТЬ
				|	ПорядокОтраженияНаСчетахУчета.СчетУчета КАК СчетУчета
				...
				Запрос.УстановитьПараметр("АналитикаУчета", Стр.Статья.ГруппаФинансовогоУчета);   
				ВыборкаЗапроса = Запрос.Выполнить().Выбрать();
				Пока ВыборкаЗапроса.Следующий() Цикл 
					СтруктураВыгрузки.Вставить("СчетУчетаРасчетов", КодСчетаРегл(ВыборкаЗапроса.СчетУчета)); 
					СтруктураВыгрузки.Вставить("Субконто1", СформироватьУИДДляВыгрузки(Стр.Статья));
				КонецЦикла;
			ИначеЕсли ЗначениеЗаполнено(Стр.НастройкаСчетовУчета) И ЗначениеЗаполнено(Стр.НастройкаСчетовУчета.СчетУчета) Тогда 
				СтруктураВыгрузки.Вставить("СчетУчетаРасчетов", КодСчетаРегл(Стр.НастройкаСчетовУчета.СчетУчета)); 
				Если ТипЗнч(Стр.НастройкаСчетовУчета.Субконто1) = Тип("СправочникСсылка.Контрагенты") Тогда 
					СтруктураВыгрузки.Вставить("Субконто1", СформироватьУИДДляВыгрузки(Стр.НастройкаСчетовУчета.Субконто1));
					СтруктураВыгрузки.Вставить("Субконто2", СформироватьУИДДляВыгрузки(?(ЗначениеЗаполнено(Стр.НастройкаСчетовУчета.Субконто2), Стр.НастройкаСчетовУчета.Субконто2, Справочники.ДоговорыКонтрагентов.ПустаяСсылка()))); 
				КонецЕсли;
			ИначеЕсли ТипЗнч(Стр.Статья) = Тип("ПланВидовХарактеристикСсылка.СтатьиДоходов") Тогда
				СтруктураВыгрузки.Вставить("СчетУчетаРасчетов", КодСчетаРегл(ПолучитьСчетУчетаИзНастроек(Перечисления.ВидыСчетовРеглУчета.Доходы, СсылкаНаОбъект.Организация, Стр.Статья, )));
			ИначеЕсли ТипЗнч(Стр.Статья) = Тип("ПланВидовХарактеристикСсылка.СтатьиРасходов") Тогда
				СтруктураВыгрузки.Вставить("СчетУчетаРасчетов", КодСчетаРегл(ПолучитьСчетУчетаИзНастроек(Перечисления.ВидыСчетовРеглУчета.Расходы, СсылкаНаОбъект.Организация, Стр.Статья, )));
			ИначеЕсли ТипЗнч(Стр.Статья) = Тип("ПланВидовХарактеристикСсылка.СтатьиАктивовПассивов") Тогда
				СтруктураВыгрузки.Вставить("СчетУчетаРасчетов", КодСчетаРегл(ПолучитьСчетУчетаИзНастроек(Перечисления.ВидыСчетовРеглУчета.ПрочиеАктивыПассивы, СсылкаНаОбъект.Организация, Стр.Статья, )));
			КонецЕсли;
		КонецЕсли; 
...
Процедура ВыгрузитьНачислениеДивидендов(СсылкаНаОбъект, ТаблицаВыгрузки, ОписаниеОшибок)
	...
	Если СсылкаНаОбъект.ФизическиеЛица.Количество() = 0 Тогда
		Возврат;
	КонецЕсли;
	
	Если СсылкаНаОбъект.ФизическиеЛица.Количество() > 1 Тогда
		ВызватьИсключение "Ожидается только одно ФЛ в начислении дивидендов";
	КонецЕсли;
	...
	СтруктураВыгрузки.Вставить("ПланируемаяДатаВыплаты",СсылкаНаОбъект.ДатаВыплаты);
	
	строкаФЛ = СсылкаНаОбъект.ФизическиеЛица[0];
	
	СтруктураВыгрузки.Вставить("Учредитель",СформироватьУИДДляВыгрузки(строкаФЛ.ФизическоеЛицо));
	СтруктураВыгрузки.Вставить("СуммаДохода",строкаФЛ.Начислено);
	СтруктураВыгрузки.Вставить("СуммаНалога",строкаФЛ.НДФЛ); 
...
Процедура ВыгрузитьВсеРасходыБудущихПериодов() Экспорт
	...
	Организация = Справочники.Организации.НайтиПоНаименованию("ИП Орлова О. Н.",Истина);
	
	Запрос.УстановитьПараметр("Период",'20210101');
	Запрос.УстановитьПараметр("СчетКт",ПланыСчетов.Хозрасчетный.ПрочиеРасходыБудущихПериодов);
	Запрос.УстановитьПараметр("Организация",Организация);
	...
	Пока Выборка.Следующий() Цикл
		...
		СтруктураВыгрузки = Новый Структура;
		СтруктураВыгрузки.Вставить("УИД", Строка(Новый УникальныйИдентификатор)); 
		СтруктураВыгрузки.Вставить("Представление", "Расходы будущих периодов за " + Выборка.Период);
		СтруктураВыгрузки.Вставить("ТипОбъекта", "ОперацияБух");
		СтруктураВыгрузки.Вставить("Дата", Выборка.Период);
		СтруктураВыгрузки.Вставить("Организация", СформироватьУИДДляВыгрузки(Организация));
		СтруктураВыгрузки.Вставить("Комментарий", "Расходы будущих периодов за " + Выборка.Период);
		СтруктураВыгрузки.Вставить("Проведен", Истина);	
		СтруктураВыгрузки.Вставить("ПометкаУдаления", Ложь);
		...
		Пока ВыборкаПроводки.Следующий() Цикл
			...
			структураПроводка.Вставить("Организация",СформироватьУИДДляВыгрузки(Организация));
			
			структураПроводка.Вставить("СчетДт",КодСчетаРегл(ПланыСчетов.Хозрасчетный.ИздержкиОбращения));
			структураПроводка.Вставить("СубконтоДт1",СформироватьУИДДляВыгрузки(ПланыВидовХарактеристик.СтатьиРасходов.НайтиПоНаименованию("Расходы будущих периодов+",Истина)));
			структураПроводка.Вставить("ВидСубконтоДт1","");
			структураПроводка.Вставить("СубконтоДт2","");
			структураПроводка.Вставить("ВидСубконтоДт2","");
			структураПроводка.Вставить("СубконтоДт3","");
			...
			структураПроводка.Вставить("ПодразделениеДт",СформироватьУИДДляВыгрузки(ВыборкаПроводки.ПодразделениеКт));
			структураПроводка.Вставить("ВалютаДт","");
				
			структураПроводка.Вставить("СчетКт",КодСчетаРегл(ПланыСчетов.Хозрасчетный.ПрочиеРасходыБудущихПериодов));
			структураПроводка.Вставить("СубконтоКт1",СформироватьУИДДляВыгрузки(ВыборкаПроводки.СубконтоКт1));
			структураПроводка.Вставить("ВидСубконтоКт1","");
			структураПроводка.Вставить("СубконтоКт2","");
			структураПроводка.Вставить("ВидСубконтоКт2","");
			структураПроводка.Вставить("СубконтоКт3","");
			...
			структураПроводка.Вставить("ПодразделениеКт",СформироватьУИДДляВыгрузки(ВыборкаПроводки.ПодразделениеКт));
			структураПроводка.Вставить("ВалютаКт","");
				
			структураПроводка.Вставить("Сумма",ВыборкаПроводки.Сумма);
			структураПроводка.Вставить("ВалютнаяСуммаДт",0);
			структураПроводка.Вставить("ВалютнаяСуммаКт",0);
			структураПроводка.Вставить("КоличествоДт",0);
			структураПроводка.Вставить("КоличествоКт",0);
			структураПроводка.Вставить("СуммаНУДт",ВыборкаПроводки.Сумма);
			структураПроводка.Вставить("СуммаНУКт",ВыборкаПроводки.Сумма);
			структураПроводка.Вставить("СуммаПРДт",0);
			структураПроводка.Вставить("СуммаПРКт",0);
			структураПроводка.Вставить("СуммаВРДт",0);
			структураПроводка.Вставить("СуммаВРКт",0);
			структураПроводка.Вставить("Содержание","Распределение РБП на расходы");
			структураПроводка.Вставить("НеКорректироватьСтоимостьАвтоматически",Ложь);
		...
		НовОб = ТаблицаВыгрузки.Добавить();
		НовОб.СсылкаНаОбъект = Документы.ОперацияБух.ПустаяСсылка();
		НовОб.СтруктураВыгрузки = СтруктураВыгрузки;
	...
	Попытка		
		ОтправитьПакет(Узел, ТаблицаВыгрузки, НомерОтправляемого);		
	Исключение	
		ЗаписьЖурналаРегистрации("Выгрузка в бухгалтерию", УровеньЖурналаРегистрации.Ошибка, , "", ОписаниеОшибки());		
	КонецПопытки;	
