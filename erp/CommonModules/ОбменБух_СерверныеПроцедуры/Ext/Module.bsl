...
Процедура ВыгрузитьСписаниеБезналичныхДенежныхСредств(СсылкаНаОбъект, ТаблицаВыгрузки, ОписаниеОшибок)
	...
	Для каждого Стр Из СсылкаНаОбъект.РасшифровкаПлатежа Цикл
		...
		Если ЗначениеЗаполнено(Стр.СтатьяРасходов) Тогда
			Нстр.Вставить("СчетСтатьиРасходов", КодСчетаРегл(ПланыСчетов.Хозрасчетный.ПрочиеРасходы)); 
			Нстр.Вставить("СтатьяРасходов", Стр.СтатьяРасходов.Наименование); 
		КонецЕсли;
...
Процедура ВыгрузитьРеализацию(СсылкаНаОбъект, ТаблицаВыгрузки, ОписаниеОшибок)
	...
	|	РеализацияТоваровУслугТовары.Цена КАК Цена,
	|	РеализацияТоваровУслугТовары.СтавкаНДС КАК СтавкаНДС,
	|	ЕСТЬNULL(ТоварыОрганизаций.НомерГТД, """") КАК НомерГТД,
	|	ЕСТЬNULL(ТоварыОрганизаций.Количество, РеализацияТоваровУслугТовары.Количество) КАК Количество,
	|	РеализацияТоваровУслугТовары.Сумма * ЕСТЬNULL(ТоварыОрганизаций.Количество, РеализацияТоваровУслугТовары.Количество) / РеализацияТоваровУслугТовары.Количество КАК Сумма,
	|	РеализацияТоваровУслугТовары.СуммаНДС * ЕСТЬNULL(ТоварыОрганизаций.Количество, РеализацияТоваровУслугТовары.Количество) / РеализацияТоваровУслугТовары.Количество КАК СуммаНДС,
	|	РеализацияТоваровУслугТовары.СуммаСНДС * ЕСТЬNULL(ТоварыОрганизаций.Количество, РеализацияТоваровУслугТовары.Количество) / РеализацияТоваровУслугТовары.Количество КАК СуммаСНДС
	|ИЗ
	...
	|		ПО (РеализацияТоваровУслугТовары.Ссылка = ТоварыОрганизаций.Регистратор)
	|			И (РеализацияТоваровУслугТовары.АналитикаУчетаНоменклатуры = ТоварыОрганизаций.АналитикаУчетаНоменклатуры)
	|
	|УПОРЯДОЧИТЬ ПО
