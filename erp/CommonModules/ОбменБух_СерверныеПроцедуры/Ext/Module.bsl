...
//!! не используется
Процедура ЗаполнитьМассивКВыгрузкеДополнительнымиЭлементами(УзлыРегистрации, Объект, ТаблицаОбъектовПланаОбмена, МассивЗначенийКРегистрацииВПланеОбмена) 
	...
	Если МассивЗначенийКРегистрацииВПланеОбмена.Найти(Объект) = Неопределено Тогда
		МассивЗначенийКРегистрацииВПланеОбмена.Добавить(Объект); 
	КонецЕсли;
	
	Для каждого РеквизитОбъекта Из МетаданныеОбъекта.Реквизиты Цикл
		
		ЗначениеРеквизита = Объект[РеквизитОбъекта.Имя];
		ТипЗначенияРеквизита = ТипЗнч(ЗначениеРеквизита);
		
		ОбъектМетаданных = Метаданные.НайтиПоТипу(ТипЗначенияРеквизита);  
		Если ОбъектМетаданных = Неопределено ИЛИ НЕ ОбщегоНазначения.СсылкаСуществует(ЗначениеРеквизита) Тогда 
			Продолжить;
		КонецЕсли;   
		
		ЕстьОбъектВСоставе = ТаблицаОбъектовПланаОбмена.НайтиСтроки(Новый Структура("ИмяОбъектаСостава", ОбъектМетаданных.Имя)).Количество() > 0;
		Если НЕ ЕстьОбъектВСоставе Тогда 
			Продолжить;
		КонецЕсли;
		
		Если МассивЗначенийКРегистрацииВПланеОбмена.Найти(ЗначениеРеквизита) = Неопределено Тогда
			//МассивЗначенийКРегистрацииВПланеОбмена.Добавить(ЗначениеРеквизита);
			//Рекурсия
			ЗаполнитьМассивКВыгрузкеДополнительнымиЭлементами(УзлыРегистрации, ЗначениеРеквизита, ТаблицаОбъектовПланаОбмена, МассивЗначенийКРегистрацииВПланеОбмена);
		КонецЕсли;
	КонецЦикла;
	...
	Для каждого ТабличнаяЧасть Из МетаданныеОбъекта.ТабличныеЧасти Цикл  
		Для каждого СтрокаТабличнойЧасти из Объект[ТабличнаяЧасть.Имя] Цикл 
			Для каждого РеквизитТабличнойЧасти Из ТабличнаяЧасть.Реквизиты Цикл 
				ЗначениеРеквизитаСтрокиТабличнойЧасти = СтрокаТабличнойЧасти[РеквизитТабличнойЧасти.Имя];
				ТипЗначенияРеквизитаСтрокиТабличнойЧасти = ТипЗнч(ЗначениеРеквизитаСтрокиТабличнойЧасти);
				
				ОбъектМетаданных = Метаданные.НайтиПоТипу(ТипЗначенияРеквизитаСтрокиТабличнойЧасти);  
				Если ОбъектМетаданных = Неопределено ИЛИ НЕ ОбщегоНазначения.СсылкаСуществует(ЗначениеРеквизитаСтрокиТабличнойЧасти) Тогда 
					Продолжить;
				КонецЕсли;         
				...
				Если МассивЗначенийКРегистрацииВПланеОбмена.Найти(ЗначениеРеквизитаСтрокиТабличнойЧасти) = Неопределено Тогда
					//МассивЗначенийКРегистрацииВПланеОбмена.Добавить(ЗначениеРеквизитаСтрокиТабличнойЧасти); 
...
Процедура ЗарегистрироватьИзменения(Объект, Узел = Неопределено) Экспорт  
	...
	Если ТипЗнч(Объект.Ссылка) = Тип("ДокументСсылка.ПриобретениеТоваровУслуг") 
		...
		//в этом случае выгружается авансовый отчёт с товароами из такого поступления	
		Возврат;	
	КонецЕсли;		
	...
	//МассивЗначенийКРегистрацииВПланеОбмена = Новый Массив;
	//ТаблицаОбъектовПланаОбмена = ПолучитьСоставПланаОбмена(УзлыРегистрации[0]);    
	//
	//Если УзлыРегистрации <> Неопределено И УзлыРегистрации.Количество() > 0 Тогда
	//	ЗаполнитьМассивКВыгрузкеДополнительнымиЭлементами(УзлыРегистрации, Объект, ТаблицаОбъектовПланаОбмена, МассивЗначенийКРегистрацииВПланеОбмена);
	//КонецЕсли;  
	//
	//Если МассивЗначенийКРегистрацииВПланеОбмена.Количество() > 0 Тогда
	//	ПланыОбмена.ЗарегистрироватьИзменения(УзлыРегистрации, МассивЗначенийКРегистрацииВПланеОбмена);
	//КонецЕсли;
...
//!! не используется
Функция ПолучитьСоставПланаОбмена(Узел) Экспорт
	
	ТаблицаОбъектовПланаОбмена = Новый ТаблицаЗначений;  
	ТаблицаОбъектовПланаОбмена.Колонки.Добавить("ВидОбъектаСостава");
	ТаблицаОбъектовПланаОбмена.Колонки.Добавить("ИмяОбъектаСостава");
	
	Для каждого ЭлСостава из Узел.Метаданные().Состав Цикл 
		НоваяСтрока = ТаблицаОбъектовПланаОбмена.Добавить();
		НоваяСтрока.ВидОбъектаСостава = ВидОбъектаМетаданныхОбмена(ЭлСостава.Метаданные);
		НоваяСтрока.ИмяОбъектаСостава = ЭлСостава.Метаданные.Имя;
	КонецЦикла;   
	
	Возврат ТаблицаОбъектовПланаОбмена; 
...
Функция НайтиОбновитьДанныеВыгрузки(СсылкаНаОбъект, ОбъектИзменен, ТаблицаВыгрузки, Организация = Неопределено, ТипОбъекта = "")
	
	ОбъектНайден = Ложь;
	СтрокиТаблицы = ТаблицаВыгрузки.НайтиСтроки(Новый Структура("СсылкаНаОбъект", СсылкаНаОбъект)); 
	Если СтрокиТаблицы.Количество() Тогда
		Если Организация = Неопределено Тогда
			Для Каждого Стр из СтрокиТаблицы Цикл
				Если ОбъектИзменен Тогда
					Стр.СтруктураВыгрузки.ОбъектИзменен = Истина;
				КонецЕсли;
				Если ТипОбъекта <> "" И Стр.СтруктураВыгрузки.ТипОбъекта = ТипОбъекта Тогда
					ОбъектНайден = Истина;
				КонецЕсли;  
			КонецЦикла;  
		...
		Иначе     
			...
			Для Каждого Стр Из СтрокиТаблицы Цикл
				Если Лев(Стр.СтруктураВыгрузки.Организация, 36) = УИДОрганизации Тогда
					ОбъектНайден = Истина;
					Если ОбъектИзменен Тогда
						Стр.СтруктураВыгрузки.ОбъектИзменен = Истина;
					КонецЕсли;  
...
//++
Процедура ВыгрузитьВалюту(СсылкаНаОбъект, ОбъектИзменен, ТаблицаВыгрузки, ОписаниеОшибок)
	
	Если СсылкаНаОбъект.Пустая() Тогда
		Возврат;
	КонецЕсли;	
	
	Если НайтиОбновитьДанныеВыгрузки(СсылкаНаОбъект, ОбъектИзменен, ТаблицаВыгрузки) Тогда
		Возврат;
	КонецЕсли; 		
	
	СтруктураВыгрузки = Новый Структура;
	СтруктураВыгрузки.Вставить("УИД", СформироватьУИДДляВыгрузки(СсылкаНаОбъект)); 
	СтруктураВыгрузки.Вставить("ОбъектИзменен", ОбъектИзменен);
	...
	Если СсылкаНаОбъект.СпособУстановкиКурса = Перечисления.СпособыУстановкиКурсаВалюты.НаценкаНаКурсДругойВалюты Тогда
		...
		//ВыгрузитьВалюту(СсылкаНаОбъект.ОсновнаяВалюта, Ложь, ТаблицаВыгрузки, ОписаниеОшибок);
...
//++
Процедура ВыгрузитьСклад(СсылкаНаОбъект, ОбъектИзменен, ТаблицаВыгрузки, ОписаниеОшибок)
	...
	Если Не СсылкаНаОбъект.ЭтоГруппа Тогда
		...
		//СтруктураВыгрузки.Вставить("Подразделение", ); //	не заданы в erp
	КонецЕсли;	
	
	СтруктураВыгрузки.Вставить("Родитель", СформироватьУИДДляВыгрузки(СсылкаНаОбъект.Родитель));
	//ВыгрузитьСклад(СсылкаНаОбъект.Родитель, Ложь, ТаблицаВыгрузки, ОписаниеОшибок);
...
//++
Процедура ВыгрузитьОрганизацию(СсылкаНаОбъект, ОбъектИзменен, ТаблицаВыгрузки, ОписаниеОшибок)
	...
	Если СсылкаНаОбъект.ГоловнаяОрганизация <> СсылкаНаОбъект Тогда
		...
		//ВыгрузитьОрганизацию(СсылкаНаОбъект.ГоловнаяОрганизация, Ложь, ТаблицаВыгрузки, ОписаниеОшибок);
	КонецЕсли;
	...
	ВыгрузитьФизЛицо(СсылкаНаОбъект.ИндивидуальныйПредприниматель, Ложь, ТаблицаВыгрузки, ОписаниеОшибок);
	...
	ВыгрузитьРегистрациюВНалоговомОргане(СсылкаНаОбъект.РегистрацияВНалоговомОргане, Ложь, ТаблицаВыгрузки, ОписаниеОшибок);
	...
	//ВыгрузитьУчетнуюЗаписьДокументооборота(СсылкаНаОбъект.УчетнаяЗаписьОбмена, Ложь, ТаблицаВыгрузки, ОписаниеОшибок);
...
//++
Процедура ВыгрузитьОрганизациюКакКонтрагента(СсылкаНаОбъект, ОбъектИзменен, ТаблицаВыгрузки, ОписаниеОшибок) //!!
	...
	Если СсылкаНаОбъект.ГоловнаяОрганизация <> СсылкаНаОбъект Тогда 
		...
		//ВыгрузитьОрганизациюКакКонтрагента(СсылкаНаОбъект.ГоловнаяОрганизация, Ложь, ТаблицаВыгрузки, ОписаниеОшибок);
	КонецЕсли;	
	...
	//ВыгрузитьБанковскийСчет(БанковскийСчет, Ложь, ТаблицаВыгрузки, ОписаниеОшибок);
	...
	//ВыгрузитьСтрану(СсылкаНаОбъект.СтранаРегистрации, Ложь, ТаблицаВыгрузки, ОписаниеОшибок);
...
//++
Процедура ВыгрузитьФизЛицо(СсылкаНаОбъект, ОбъектИзменен, ТаблицаВыгрузки, ОписаниеОшибок)
	...
	Если НЕ СсылкаНаОбъект.ЭтоГруппа Тогда
		...
		//СтруктураВыгрузки.Вставить("ГруппаДоступа", СсылкаНаОбъект.ГруппаДоступа); 
		...
		//СтруктураВыгрузки.Вставить("ОсновнойБанковскийСчет", СсылкаНаОбъект.ОсновнойБанковскийСчет);	
	...
	СтруктураВыгрузки.Вставить("Родитель", СформироватьУИДДляВыгрузки(СсылкаНаОбъект.Родитель));
	//ВыгрузитьФизЛицо(СсылкаНаОбъект.Родитель, Ложь, ТаблицаВыгрузки, ОписаниеОшибок);
...
//!!!
Процедура ВыгрузитьПользователя(СсылкаНаОбъект, ОбъектИзменен, ТаблицаВыгрузки, ОписаниеОшибок)
...
Процедура ВыгрузитьПодразделение(СсылкаНаОбъект, Организация, ОбъектИзменен, ТаблицаВыгрузки, ОписаниеОшибок)
	
	Если СсылкаНаОбъект.Пустая() Тогда
		Возврат;
	КонецЕсли;	 
	
	Если НайтиОбновитьДанныеВыгрузки(СсылкаНаОбъект, ОбъектИзменен, ТаблицаВыгрузки, Организация) Тогда
		Возврат;
	КонецЕсли;  
	...
	СтруктураВыгрузки.Вставить("Наименование", СсылкаНаОбъект.Наименование);
	СтруктураВыгрузки.Вставить("Родитель", СформироватьУИДДляВыгрузки(СсылкаНаОбъект.Родитель));
	//ВыгрузитьПодразделение(СсылкаНаОбъект.Родитель, Организация, Ложь, ТаблицаВыгрузки, ОписаниеОшибок);
...
//++
Процедура ВыгрузитьКонтрагента(СсылкаНаОбъект, ОбъектИзменен, ТаблицаВыгрузки, ОписаниеОшибок)
	...
	Если СсылкаНаОбъект.ГоловнойКонтрагент <> СсылкаНаОбъект Тогда
		...
		//ВыгрузитьКонтрагента(СсылкаНаОбъект.ГоловнойКонтрагент, Ложь, ТаблицаВыгрузки, ОписаниеОшибок);
	КонецЕсли;
	СтруктураВыгрузки.Вставить("КПП", СсылкаНаОбъект.КПП);
	СтруктураВыгрузки.Вставить("ИНН", СсылкаНаОбъект.ИНН);
	СтруктураВыгрузки.Вставить("КодПоОКПО", СсылкаНаОбъект.КодПоОКПО);
	//СтруктураВыгрузки.Вставить("ОсновнойБанковскийСчет", БанковскийСчет.УникальныйИдентификатор()); //!! нужно ли определять основной БС контрагента
	////ВыгрузитьБанковскийСчет(БанковскийСчет, Ложь, ТаблицаВыгрузки, ОписаниеОшибок);
...
//++
Процедура ВыгрузитьДоговор(СсылкаНаОбъект, ОбъектИзменен, ТаблицаВыгрузки, ОписаниеОшибок)
	...
	//ВыгрузитьВалюту(СсылкаНаОбъект.ВалютаВзаиморасчетов, Ложь, ТаблицаВыгрузки, ОписаниеОшибок); 
	
	СтруктураВыгрузки.Вставить("Организация", СформироватьУИДДляВыгрузки(СсылкаНаОбъект.Организация)); 
	//ВыгрузитьОрганизацию(СсылкаНаОбъект.Организация, Ложь, ТаблицаВыгрузки, ОписаниеОшибок); 
	...
	Если ТипЗнч(СсылкаНаОбъект) = Тип("СправочникСсылка.ДоговорыМеждуОрганизациями") Тогда
		...
		//ВыгрузитьОрганизациюКакКонтрагента(СсылкаНаОбъект.ОрганизацияПолучатель, Ложь, ТаблицаВыгрузки, ОписаниеОшибок);
	Иначе
		...
		//ВыгрузитьКонтрагента(СсылкаНаОбъект.Контрагент, Ложь, ТаблицаВыгрузки, ОписаниеОшибок);
...
//++
Процедура ВыгрузитьРегистрациюВНалоговомОргане(СсылкаНаОбъект, ОбъектИзменен, ТаблицаВыгрузки, ОписаниеОшибок)
	...
	//ВыгрузитьОрганизацию(СсылкаНаОбъект.Владелец, Ложь, ТаблицаВыгрузки, ОписаниеОшибок); //организация уже должна быть в пачке выгружаемых данных
	
	//СтруктураВыгрузки.Вставить("Доверенность", Строка(СсылкаНаОбъект.Доверенность.УникальныйИдентификатор())); //2 типа
	...
	//СтруктураВыгрузки.Вставить("Представитель", СсылкаНаОбъект.Представитель); // не используется
	СтруктураВыгрузки.Вставить("УполномоченноеЛицоПредставителя", СсылкаНаОбъект.УполномоченноеЛицоПредставителя);
	
	//СтруктураВыгрузки.Вставить("КодРегиона", СсылкаНаОбъект.КодРегиона);//!! этих реквизитов нет в ERP
	//СтруктураВыгрузки.Вставить("НаименованиеСлужебное", СсылкаНаОбъект.НаименованиеСлужебное);
	//СтруктураВыгрузки.Вставить("ДатаПостановкиНаУчет", СсылкаНаОбъект.ДатаПостановкиНаУчет);
	//СтруктураВыгрузки.Вставить("ДатаСнятияСУчета", СсылкаНаОбъект.ДатаСнятияСУчета);
...
//++
Процедура ВыгрузитьУчетнуюЗаписьДокументооборота(СсылкаНаОбъект, ОбъектИзменен, ТаблицаВыгрузки, ОписаниеОшибок)
	...
	//ВыгрузитьНалоговыйОрган(СсылкаНаОбъект.НалоговыйОрган, Ложь, ТаблицаВыгрузки, ОписаниеОшибок);
	...
	//ВыгрузитьСерверДокументооборота(СсылкаНаОбъект.СерверДокументооборота, Ложь, ТаблицаВыгрузки, ОписаниеОшибок);
...
//++
Процедура ВыгрузитьСерверДокументооборота(СсылкаНаОбъект, ОбъектИзменен, ТаблицаВыгрузки, ОписаниеОшибок)  
...
//++
Процедура ВыгрузитьНалоговыйОрган(СсылкаНаОбъект, ОбъектИзменен, ТаблицаВыгрузки, ОписаниеОшибок)
	...
	СтруктураВыгрузки.Вставить("ПометкаУдаления", СсылкаНаОбъект.ПометкаУдаления); 
	СтруктураВыгрузки.Вставить("Родитель", СформироватьУИДДляВыгрузки(СсылкаНаОбъект.Родитель));
	//ВыгрузитьНалоговыйОрган(СсылкаНаОбъект.Родитель, Ложь, ТаблицаВыгрузки, ОписаниеОшибок);
	
	//СтруктураВыгрузки.Вставить("Вид", Строка(СсылкаНаОбъект.Доверенность.УникальныйИдентификатор())); // ВидыНалоговыхОрганов
...
//++
Процедура ВыгрузитьСтрану(СсылкаНаОбъект, ОбъектИзменен, ТаблицаВыгрузки, ОписаниеОшибок)
	...
	//!!учесть, что Россия в обеих базах предопределена, можно выгружать по наименованию (но нужно фиксировать в регистре всё равно)
...
//++
//!! особый порядок запроса объектов
Процедура ВыгрузитьБанковскийСчет(СсылкаНаОбъект, ОбъектИзменен, ТаблицаВыгрузки, ОписаниеОшибок, ВыгружатьКакБСКонтрагента = Ложь)
	...
	Если ТипЗнч(СсылкаНаОбъект.Владелец) = Тип("СправочникСсылка.Организации") И НЕ ВыгружатьКакБСКонтрагента Тогда //!! надо определять при запросе доп.объекта
		...
		//ВыгрузитьОрганизацию(СсылкаНаОбъект.Владелец, Ложь, ТаблицаВыгрузки, ОписаниеОшибок);
		//СтруктураВыгрузки.Вставить("ПодразделениеОрганизации", Строка(СсылкаНаОбъект.Подразделение.УникальныйИдентификатор()));//нужно только для счета организации, не заполняется в erp
	...
	Иначе	  
		...
		//ВыгрузитьКонтрагента(СсылкаНаОбъект.Владелец, Ложь, ТаблицаВыгрузки, ОписаниеОшибок);
		СтруктураВыгрузки.Вставить("ГосударственныйКонтракт", СсылкаНаОбъект.ГосударственныйКонтракт);
	КонецЕсли;	
	...
	//ВыгрузитьБанк(СсылкаНаОбъект.Банк, Ложь, ТаблицаВыгрузки, ОписаниеОшибок);
	
	//СтруктураВыгрузки.Вставить("Валютный", Ложь); //если не рубли, то валютный
	СтруктураВыгрузки.Вставить("ВалютаДенежныхСредств", СформироватьУИДДляВыгрузки(СсылкаНаОбъект.ВалютаДенежныхСредств));
	//ВыгрузитьВалюту(СсылкаНаОбъект.ВалютаДенежныхСредств, Ложь, ТаблицаВыгрузки, ОписаниеОшибок);
	
	//СтруктураВыгрузки.Вставить("НомерИДатаРазрешения", СсылкаНаОбъект.НомерИДатаРазрешения);
	//СтруктураВыгрузки.Вставить("ДатаОткрытия", СсылкаНаОбъект.ДатаОткрытия);
	//СтруктураВыгрузки.Вставить("ДатаЗакрытия", СсылкаНаОбъект.ДатаЗакрытия);
	СтруктураВыгрузки.Вставить("БанкДляРасчетов", СформироватьУИДДляВыгрузки(СсылкаНаОбъект.БанкДляРасчетов));
	//ВыгрузитьБанк(СсылкаНаОбъект.БанкДляРасчетов, Ложь, ТаблицаВыгрузки, ОписаниеОшибок);
	
	//СтруктураВыгрузки.Вставить("ВидСчета", ""); //
...
//++
Процедура ВыгрузитьБанк(СсылкаНаОбъект, ОбъектИзменен, ТаблицаВыгрузки, ОписаниеОшибок)
	...
	СтруктураВыгрузки.Вставить("ПометкаУдаления", СсылкаНаОбъект.ПометкаУдаления);	
	СтруктураВыгрузки.Вставить("Родитель", СформироватьУИДДляВыгрузки(СсылкаНаОбъект.Родитель));
	//ВыгрузитьБанк(СсылкаНаОбъект.Родитель, Ложь, ТаблицаВыгрузки, ОписаниеОшибок);	
	СтруктураВыгрузки.Вставить("ЭтоГруппа", СсылкаНаОбъект.ЭтоГруппа);
	Если Не СсылкаНаОбъект.ЭтоГруппа Тогда
		...
		//ВыгрузитьСтрану(СсылкаНаОбъект.Страна, Ложь, ТаблицаВыгрузки, ОписаниеОшибок);
...
//++
Процедура ВыгрузитьНоменклатуру(СсылкаНаОбъект, ОбъектИзменен, ТаблицаВыгрузки, ОписаниеОшибок)
	...
	СтруктураВыгрузки.Вставить("ПометкаУдаления", СсылкаНаОбъект.ПометкаУдаления);	
	СтруктураВыгрузки.Вставить("Родитель", СформироватьУИДДляВыгрузки(СсылкаНаОбъект.Родитель));
	//ВыгрузитьНоменклатуру(СсылкаНаОбъект.Родитель, Ложь, ТаблицаВыгрузки, ОписаниеОшибок);	
	...
	Если Не СсылкаНаОбъект.ЭтоГруппа Тогда
		...
		//ВыгрузитьЕдиницуИзмерения(СсылкаНаОбъект.ЕдиницаИзмерения, Ложь, ТаблицаВыгрузки, ОписаниеОшибок);
		
		СтруктураВыгрузки.Вставить("Артикул", СсылкаНаОбъект.Артикул);
		СтруктураВыгрузки.Вставить("НаименованиеПолное", СсылкаНаОбъект.НаименованиеПолное);
		//СтруктураВыгрузки.Вставить("КодТНВЭД", СсылкаНаОбъект.КодТНВЭД); //!! выгружать справочники или искать/создавать по кодам
		//СтруктураВыгрузки.Вставить("КодОКВЭД", СсылкаНаОбъект.КодОКВЭД); //!! выгружать справочники или искать/создавать по кодам
		//СтруктураВыгрузки.Вставить("КодОКП", СсылкаНаОбъект.КодОКП); //!! выгружать справочники или искать/создавать по кодам
		//СтруктураВыгрузки.Вставить("КодОКВЭД2", СсылкаНаОбъект.КодОКВЭД2); //!! выгружать справочники или искать/создавать по кодам
		//СтруктураВыгрузки.Вставить("КодОКПД2", СсылкаНаОбъект.КодОКПД2); //!! выгружать справочники или искать/создавать по кодам
...
Процедура ВыгрузитьЕдиницуИзмерения(СсылкаНаОбъект, ОбъектИзменен, ТаблицаВыгрузки, ОписаниеОшибок)
...
Процедура ВыгрузитьСтатьюРасходов(СсылкаНаОбъект, ОбъектИзменен, ТаблицаВыгрузки, ОписаниеОшибок)
...
Процедура ВыгрузитьСтатьюДоходов(СсылкаНаОбъект, ОбъектИзменен, ТаблицаВыгрузки, ОписаниеОшибок)
...
Процедура ВыгрузитьРезервыОценочныеОбязательства(СсылкаНаОбъект, ОбъектИзменен, ТаблицаВыгрузки, ОписаниеОшибок)
...
Процедура ВыгрузитьЦенныеБумаги(СсылкаНаОбъект, ОбъектИзменен, ТаблицаВыгрузки, ОписаниеОшибок)
	...
	СтруктураВыгрузки.Вставить("ЭтоГруппа", СсылкаНаОбъект.ЭтоГруппа);
	СтруктураВыгрузки.Вставить("Родитель", СформироватьУИДДляВыгрузки(СсылкаНаОбъект.Родитель));
	//ВыгрузитьЦенныеБумаги(СсылкаНаОбъект.Родитель, Ложь, ТаблицаВыгрузки, ОписаниеОшибок);
...
//++
Процедура ВыгрузитьСтатьюДДС(СсылкаНаОбъект, ОбъектИзменен, ТаблицаВыгрузки, ОписаниеОшибок)
	...
	СтруктураВыгрузки.Вставить("ПометкаУдаления", СсылкаНаОбъект.ПометкаУдаления);	
	СтруктураВыгрузки.Вставить("Родитель", СформироватьУИДДляВыгрузки(СсылкаНаОбъект.Родитель));
	//ВыгрузитьСтатьюДДС(СсылкаНаОбъект.Родитель, Ложь, ТаблицаВыгрузки, ОписаниеОшибок);	
...
Процедура ВыгрузитьОбъектЭксплуатации(СсылкаНаОбъект, ОбъектИзменен, ТаблицаВыгрузки, ОписаниеОшибок)
	...
	//
	СтруктураВыгрузки.Вставить("Родитель", СформироватьУИДДляВыгрузки(СсылкаНаОбъект.Родитель));   
	//ВыгрузитьОбъектСтроительства(СсылкаНаОбъект.Родитель, Ложь, ТаблицаВыгрузки, ОписаниеОшибок); //!!
	//
	СтруктураВыгрузки.Вставить("Наименование", СсылкаНаОбъект.Наименование);  
	СтруктураВыгрузки.Вставить("Код", СсылкаНаОбъект.Код);  
...
Процедура ВыгрузитьОбъектСтроительства(СсылкаНаОбъект, ОбъектИзменен, ТаблицаВыгрузки, ОписаниеОшибок)
...
Процедура ВыгрузитьНематериальныйАктив(СсылкаНаОбъект, ОбъектИзменен, ТаблицаВыгрузки, ОписаниеОшибок)
	...
	//     
	СтруктураВыгрузки.Вставить("Родитель", СформироватьУИДДляВыгрузки(СсылкаНаОбъект.Родитель));   
	//ВыгрузитьНематериальныйАктив(СсылкаНаОбъект.Родитель, Ложь, ТаблицаВыгрузки, ОписаниеОшибок);
...
Процедура ВыгрузитьСотрудника(СсылкаНаОбъект, ОбъектИзменен, ТаблицаВыгрузки, ОписаниеОшибок)
	...
	//ВыгрузитьФизЛицо(СсылкаНаОбъект.ФизическоеЛицо, Ложь, ТаблицаВыгрузки, ОписаниеОшибок);                             
	...
	Если СсылкаНаОбъект.ГоловнойСотрудник <> СсылкаНаОбъект Тогда
		...
		//ВыгрузитьСотрудника(СсылкаНаОбъект.ГоловнойСотрудник, Ложь, ТаблицаВыгрузки, ОписаниеОшибок);                             
	КонецЕсли;	
...
//++
Процедура ВыгрузитьАвансовыйОтчет(СсылкаНаОбъект, ОбъектИзменен, ТаблицаВыгрузки, ОписаниеОшибок)
	...
	//ВыгрузитьВалюту(СсылкаНаОбъект.Валюта, Ложь, ТаблицаВыгрузки, ОписаниеОшибок);
	
	//СтруктураВыгрузки.Вставить("Склад", Строка(СсылкаНаОбъект.Организация.УникальныйИдентификатор()));
	//ВыгрузитьСклад(СсылкаНаОбъект.Склад, Ложь, ТаблицаВыгрузки, ОписаниеОшибок);		
	
	СтруктураВыгрузки.Вставить("Организация", СформироватьУИДДляВыгрузки(СсылкаНаОбъект.Организация));
	//ВыгрузитьОрганизацию(СсылкаНаОбъект.Организация, Ложь, ТаблицаВыгрузки, ОписаниеОшибок);	
	
	СтруктураВыгрузки.Вставить("ПодотчетноеЛицо", СформироватьУИДДляВыгрузки(СсылкаНаОбъект.ПодотчетноеЛицо)); 
	//ВыгрузитьФизЛицо(СсылкаНаОбъект.ПодотчетноеЛицо, Ложь, ТаблицаВыгрузки, ОписаниеОшибок);	
	
	СтруктураВыгрузки.Вставить("Автор", СформироватьУИДДляВыгрузки(СсылкаНаОбъект.Автор)); 
	//ВыгрузитьПользователя(СсылкаНаОбъект.Автор, Ложь, ТаблицаВыгрузки, ОписаниеОшибок);	
	
	СтруктураВыгрузки.Вставить("Подразделение", СформироватьУИДДляВыгрузки(СсылкаНаОбъект.Подразделение)); 
	//ВыгрузитьПодразделение(СсылкаНаОбъект.Подразделение, СсылкаНаОбъект.Организация, Ложь, ТаблицаВыгрузки, ОписаниеОшибок);	
	...
	Для Каждого Стр Из СсылкаНаОбъект.ОплатаПоставщикам Цикл
		...
		//ВыгрузитьКонтрагента(Стр.ОбъектРасчетов.Контрагент, Ложь, ТаблицаВыгрузки, ОписаниеОшибок);	
		
		Строка.Вставить("Договор", СформироватьУИДДляВыгрузки(Стр.ОбъектРасчетов.Договор)); //!!как быть с пустыми договорами?
		//ВыгрузитьДоговор(Стр.ОбъектРасчетов.Договор, Ложь, ТаблицаВыгрузки, ОписаниеОшибок);
...
//!!
Процедура ВыгрузитьВнутреннееПотреблениеТоваров(СсылкаНаОбъект, ОбъектИзменен, ТаблицаВыгрузки, ОписаниеОшибок) 
	...
	Если  СсылкаНаОбъект.ХояйственнаяОперация =  Перечисления.ХозяйственныеОперации.СписаниеТоваровПоТребованию Тогда
		...
		//СтруктураВыгрузки.Вставить("ВидОперации", "Использование материалов");
	Иначе 
...
//!!
Процедура ВыгрузитьВзаимозачетЗадолженности(СсылкаНаОбъект, ОбъектИзменен, ТаблицаВыгрузки, ОписаниеОшибок)
...
//++!!
Процедура ВыгрузитьВозвратТоваровОтКлиента(СсылкаНаОбъект, ОбъектИзменен, ТаблицаВыгрузки, ОписаниеОшибок)
	...
	//ВыгрузитьДоговор(СсылкаНаОбъект.Договор, Ложь, ТаблицаВыгрузки, ОписаниеОшибок);
	...
	//ВыгрузитьПользователя(СсылкаНаОбъект.Менеджер, Ложь, ТаблицаВыгрузки, ОписаниеОшибок);
	
	СтруктураВыгрузки.Вставить("ДокументРеализации", СформироватьУИДДляВыгрузки(СсылкаНаОбъект.ДокументРеализации)); 
	//ВыгрузитьРеализацию(СсылкаНаОбъект.ДокументРеализации, Ложь, ТаблицаВыгрузки, ОписаниеОшибок); // может быть также ОРП, но они не используются в erp
	...
	Для Каждого Стр из СсылкаНаОбъект.Товары Цикл
		...
		//ВыгрузитьНоменклатуру(Стр.Номенклатура, Ложь, ТаблицаВыгрузки, ОписаниеОшибок);//!!
		...
		//ВыгрузитьСтрану(Стр.НомерГТД.СтранаПроисхождения, Ложь, ТаблицаВыгрузки, ОписаниеОшибок);	
		НСтр.Вставить("НомерГТД", НомерГТД);
		//НСтр.Вставить("СпособУчетаНДС", ПринимаетсяКВычету);//!!??		
...
//+-
Процедура ВыгрузитьВозвратТоваровПоставщику(СсылкаНаОбъект, ОбъектИзменен, ТаблицаВыгрузки, ОписаниеОшибок)
	...
	СтруктураВыгрузки.Вставить("КурсВзаиморасчетов", 1); 
	СтруктураВыгрузки.Вставить("КратностьВзаиморасчетов", 1); 
	//СтруктураВыгрузки.Вставить("СчетУчетаРасчетовСКонтрагентом", СсылкаНаОбъект.ЦенаВключаетНДС); 
	//СтруктураВыгрузки.Вставить("СчетУчетаРасчетовПоПретензиям", СсылкаНаОбъект.ЦенаВключаетНДС); 
	//СтруктураВыгрузки.Вставить("СчетУчетаНДС", СсылкаНаОбъект.ЦенаВключаетНДС); 
	//ВидОперации!!??
	...
	//СтруктураВыгрузки.Вставить("Грузоотправитель", Строка(СсылкаНаОбъект.Грузоотправитель.УникальныйИдентификатор())); //не используются
	//СтруктураВыгрузки.Вставить("Грузополучатель", Строка(СсылкаНаОбъект.Грузополучатель.УникальныйИдентификатор())); 
	...
	ВыгрузитьПриобретение(СсылкаНаОбъект.ДокументПоступления, Ложь, ТаблицаВыгрузки, ОписаниеОшибок);	
	Товары = Новый Массив;
	Для Каждого Стр Из СсылкаНаОбъект.Товары Цикл
		НСтр = Новый Структура;
		НСтр.Вставить("Номенклатура", СформироватьУИДДляВыгрузки(Стр.Номенклатура));		
		...
		//!!счета учета номенклатуры определять по группе финансового учёта 
		//НСтр.Вставить("СчетРасчетов",Стр.);   //!!
		//НСтр.Вставить("СчетЗатрат",Стр.);   //!!
		//НСтр.Вставить("СчетУчета",Стр.);  //!!
		//НСтр.Вставить("СчетУчетаНДС",Стр.); //!!
		
		НомерГТД = Новый Структура("СтранаПроисхождения, Код, ПометкаУдаления, РегистрационныйНомер, СтранаВвозаНеРФ"); //!!
		ЗаполнитьЗначенияСвойств(НомерГТД, Стр.НомерГТД);
		НомерГТД.СтранаПроисхождения = СформироватьУИДДляВыгрузки(Стр.НомерГТД.СтранаПроисхождения);
		//ВыгрузитьСтрану(Стр.НомерГТД.СтранаПроисхождения, Ложь, ТаблицаВыгрузки, ОписаниеОшибок);	
		НСтр.Вставить("НомерГТД", НомерГТД); // тут будет сразу и страна, в БП нужно будет найти/создать ГТД по реквизитам и найти/обновить страну по УИДу
		
		//!!номера ГТД с количеством/суммой и т.д. брать из тч Виды запасов
...
//++
Процедура ВыгрузитьНачислениеДивидендов(СсылкаНаОбъект, ОбъектИзменен, ТаблицаВыгрузки, ОписаниеОшибок)
	...
	//ВыгрузитьПользователя(СсылкаНаОбъект.Ответственный, Ложь, ТаблицаВыгрузки, ОписаниеОшибок);
	
	СтруктураВыгрузки.Вставить("ДатаВыплаты", СсылкаНаОбъект.ДатаВыплаты);
	
	//СтруктураВыгрузки.Вставить("ТипУчредителя", физлицо);
...
//!!
Процедура ВыгрузитьОперациюБух(СсылкаНаОбъект, ОбъектИзменен, ТаблицаВыгрузки, ОписаниеОшибок)
	...
	Для Каждого Стр Из ХозрасчетныйРез Цикл
		...
		//ВыгрузитьВалюту(Стр.ВалютаДт, Ложь, ТаблицаВыгрузки, ОписаниеОшибок);
		//ВыгрузитьВалюту(Стр.ВалютаКт, Ложь, ТаблицаВыгрузки, ОписаниеОшибок);
		//ВыгрузитьПодразделение(Стр.ПодразделениеДт, СсылкаНаОбъект.Организация, Ложь, ТаблицаВыгрузки, ОписаниеОшибок);
		//ВыгрузитьПодразделение(Стр.ПодразделениеКт, СсылкаНаОбъект.Организация, Ложь, ТаблицаВыгрузки, ОписаниеОшибок);
	...
	Для Каждого Стр Из НДСЗаписиКнигиПокупокРез Цикл
		...
		Иначе
			...
			//Если ТипЗнч(Стр.Поставщик) = Тип("СправочникСсылка.Организации") Тогда
			//	ВыгрузитьОрганизациюКакКонтрагента(Стр.Поставщик, Ложь, ТаблицаВыгрузки, ОписаниеОшибок);
			//Иначе
			//	ВыгрузитьКонтрагента(Стр.Поставщик, Ложь, ТаблицаВыгрузки, ОписаниеОшибок);
			//КонецЕсли;	
		КонецЕсли;
	...
	Для Каждого Стр Из ЖурналУчетаСчетовФактурРез Цикл
		...
		//ВыгрузитьКонтрагента(Стр.Контрагент, Ложь, ТаблицаВыгрузки, ОписаниеОшибок);
		...
		//!!ВыгрузитьСчетФактуруПолученный(Стр.СчетФактураПолученныйОтПродавца, Ложь, ТаблицаВыгрузки, ОписаниеОшибок);
		...
		//ВыгрузитьВалюту(Стр.Валюта, Ложь, ТаблицаВыгрузки, ОписаниеОшибок);
		...
		//ВыгрузитьКонтрагента(Стр.Продавец, Ложь, ТаблицаВыгрузки, ОписаниеОшибок);
		...
		Иначе
			...
			//Если ТипЗнч(Стр.Посредник) = Тип("СправочникСсылка.Организации") Тогда
			//	ВыгрузитьОрганизациюКакКонтрагента(Стр.Посредник, Ложь, ТаблицаВыгрузки, ОписаниеОшибок);
			//Иначе     
			//	ВыгрузитьКонтрагента(Стр.Посредник, Ложь, ТаблицаВыгрузки, ОписаниеОшибок);
			//КонецЕсли;	
		КонецЕсли;	
		...
		Иначе
			...
			//Если ТипЗнч(Стр.Субкомиссионер) = Тип("СправочникСсылка.Организации") Тогда
			//	ВыгрузитьОрганизациюКакКонтрагента(Стр.Субкомиссионер, Ложь, ТаблицаВыгрузки, ОписаниеОшибок);
			//Иначе     
			//	ВыгрузитьКонтрагента(Стр.Субкомиссионер, Ложь, ТаблицаВыгрузки, ОписаниеОшибок);
			//КонецЕсли;	
		КонецЕсли;			
...
Процедура ВыгрузитьОприходованиеИзлишков(СсылкаНаОбъект, ОбъектИзменен, ТаблицаВыгрузки, ОписаниеОшибок)
	...
	КонецЕсли;	
	//СтруктураВыгрузки.Вставить("ПересчетТоваров", СформироватьУИДДляВыгрузки(СсылкаНаОбъект.ПересчетТоваров)); //если потребуется выгружать пересчеты товаров  - они же инвентаризации
...
Процедура ВыгрузитьСписаниеНедостач(СсылкаНаОбъект, ОбъектИзменен, ТаблицаВыгрузки, ОписаниеОшибок)
...
Процедура ВыгрузитьОперациюПоПлатежнойКарте(СсылкаНаОбъект, ОбъектИзменен, ТаблицаВыгрузки, ОписаниеОшибок)
	...
	СтруктураВыгрузки.Вставить("ОбъектИзменен", ОбъектИзменен);
	СтруктураВыгрузки.Вставить("Представление", Строка(СсылкаНаОбъект));
	СтруктураВыгрузки.Вставить("ТипОбъекта", "ЭкварийнговаяОперация");
	СтруктураВыгрузки.Вставить("Проведен", СсылкаНаОбъект.Проведен);	
	СтруктураВыгрузки.Вставить("ПометкаУдаления", СсылкаНаОбъект.ПометкаУдаления);
	СтруктураВыгрузки.Вставить("Комментарий", СсылкаНаОбъект.Комментарий);                                       
...
//!!
Процедура ВыгрузитьПоступлениеБезналичныхДС(СсылкаНаОбъект, ОбъектИзменен, ТаблицаВыгрузки, ОписаниеОшибок)
	...
	СтруктураВыгрузки.Вставить("ОбъектИзменен", ОбъектИзменен);
	СтруктураВыгрузки.Вставить("Представление", Строка(СсылкаНаОбъект));
	СтруктураВыгрузки.Вставить("ТипОбъекта", "ПоступлениеБезналичныхДС");
	СтруктураВыгрузки.Вставить("Проведен", СсылкаНаОбъект.Проведен);	
	СтруктураВыгрузки.Вставить("ПометкаУдаления", СсылкаНаОбъект.ПометкаУдаления);
	СтруктураВыгрузки.Вставить("СуммаДокумента", СсылкаНаОбъект.СуммаОперации);
	СтруктураВыгрузки.Вставить("Комментарий", СсылкаНаОбъект.Комментарий);                                       
	...
	СтруктураВыгрузки.Вставить("СчетБанк", "51"); //!!может быть 76.02, 60.01, 60.02 и пустой                                      
	...
	//ВыгрузитьБанковскийСчет(СсылкаНаОбъект.БанковскийСчет, Ложь, ТаблицаВыгрузки, ОписаниеОшибок);
	...
	//ВыгрузитьСтатьюДДС(СсылкаНаОбъект.СтатьяДвиженияДенежныхСредств, Ложь, ТаблицаВыгрузки, ОписаниеОшибок);
	
	Если Не ЗначениеЗаполнено(СсылкаНаОбъект.БанковскийСчетОтправитель) Тогда
		
		СтруктураВыгрузки.Вставить("Контрагент", СформироватьУИДДляВыгрузки(СсылкаНаОбъект.Контрагент));
		//ВыгрузитьКонтрагента(СсылкаНаОбъект.Контрагент, Ложь, ТаблицаВыгрузки, ОписаниеОшибок);
		
		СтруктураВыгрузки.Вставить("БанковскийСчетКонтрагента", СформироватьУИДДляВыгрузки(СсылкаНаОбъект.БанковскийСчетКонтрагента));
		//ВыгрузитьБанковскийСчет(СсылкаНаОбъект.БанковскийСчетКонтрагента, Ложь, ТаблицаВыгрузки, ОписаниеОшибок);
		
	Иначе
		
		СтруктураВыгрузки.Вставить("Контрагент", СформироватьУИДДляВыгрузки(СсылкаНаОбъект.БанковскийСчетОтправитель.Владедец));
		//ВыгрузитьОрганизациюКакКонтрагента(СсылкаНаОбъект.БанковскийСчетОтправитель.Владедец, Ложь, ТаблицаВыгрузки, ОписаниеОшибок);
		
		СтруктураВыгрузки.Вставить("БанковскийСчетКонтрагента", СформироватьУИДДляВыгрузки(СсылкаНаОбъект.БанковскийСчетОтправитель));
		//ВыгрузитьБанковскийСчет(СсылкаНаОбъект.БанковскийСчетОтправитель, Ложь, ТаблицаВыгрузки, ОписаниеОшибок);
		
	КонецЕсли;
	
	Если ЗначениеЗаполнено(СсылкаНаОбъект.ДоговорЭквайринга) Тогда
		СтруктураВыгрузки.Вставить("Договор", СформироватьУИДДляВыгрузки(СсылкаНаОбъект.ДоговорЭквайринга));
		//ВыгрузитьДоговор(СсылкаНаОбъект.ДоговорЭквайринга, Ложь, ТаблицаВыгрузки, ОписаниеОшибок);
	Иначе
		СтруктураВыгрузки.Вставить("Договор", СформироватьУИДДляВыгрузки(СсылкаНаОбъект.Договор));
		//ВыгрузитьДоговор(СсылкаНаОбъект.Договор, Ложь, ТаблицаВыгрузки, ОписаниеОшибок);
	КонецЕсли;
	...
	ХозОперация = СсылкаНаОбъект.ХозяйственнаяОперация; 
	ВидОперации = "";
	Если ХозОперация = Перечисления.ХозяйственныеОперации.ПоступлениеОплатыОтКлиента 
		ИЛИ ХозОперация = Перечисления.ХозяйственныеОперации.ПоступлениеДенежныхСредствИзДругойОрганизации Тогда 
		ВидОперации = "ОплатаПокупателя";
		//если статья ДДС поступление залога, то счёт 76.09
		//иначе счета 62.01 и 62.02
	ИначеЕсли ХозОперация = Перечисления.ХозяйственныеОперации.ВозвратДенежныхСредствОтПоставщика
		ИЛИ ХозОперация = Перечисления.ХозяйственныеОперации.ВозвратДенежныхСредствОтДругойОрганизации Тогда
		ВидОперации = "ВозвратОтПоставщика";
	ИначеЕсли ХозОперация = Перечисления.ХозяйственныеОперации.ПоступлениеОплатыПоПлатежнойКарте Тогда
		ВидОперации = "ПоступленияОтПродажПоПлатежнымКартамИБанковскимКредитам"
	ИначеЕсли ХозОперация = Перечисления.ХозяйственныеОперации.ПоступлениеДенежныхСредствСДругогоСчета Тогда
		ВидОперации = "ПереводСДругогоСчета";
	ИначеЕсли ХозОперация = Перечисления.ХозяйственныеОперации.ИнкассацияДенежныхСредствВБанк Тогда
		ВидОперации = "Инкассация";
		//статья ддс проставляетя при формировании движений
	ИначеЕсли ХозОперация = Перечисления.ХозяйственныеОперации.ПрочееПоступлениеДенежныхСредств Тогда
		ВидОперации = "ПрочееПоступление";
	Иначе       
		ОписаниеОшибок.Добавить("Для операции " + ХозОперация + " не настроена выгрузка поступлений бехналичных ДС." + Символы.ПС + СсылкаНаОбъект);
		Возврат;
	КонецЕсли;		 
	
	СтруктураВыгрузки.Вставить("ВидОперации", ВидОперации);
	...
	Для каждого Стр Из СсылкаНаОбъект.РасшифровкаПлатежа Цикл
		...
		НСтр.Вставить("СуммаВзаиморасчетов", Стр.СуммаВзаиморасчетов); 
		НСтр.Вставить("СтавкаНДС", XMLСтрока(Стр.СтавкаНДС.ПеречислениеСтавкаНДС));
		НСтр.СуммаНДС("СуммаНДС", Стр.СуммаНДС);	
		НСтр.Вставить("СтатьяДвиженияДенежныхСредств", СформироватьУИДДляВыгрузки(Стр.СтатьяДвиженияДенежныхСредств));
		//ВыгрузитьСтатьюДДС(Стр.СтатьяДвиженияДенежныхСредств, Ложь, ТаблицаВыгрузки, ОписаниеОшибок);
		//!!СпособПогашенияЗадолженности -автоматически?
		//Сделка - документ расчетоы
		//СчетНаОплату - заказ клиента или заявка на возврат
		//НСтр.Вставить("ДоговорКонтрагента", Строка(Стр.ОбъектРасчетов.Договор.УникальныйИдентификатор())); //возможно, для поступлений не нужен
		//ВыгрузитьДоговор(Стр.ДоговорКонтрагента, Ложь, ТаблицаВыгрузки, ОписаниеОшибок);
		//СчетУчетаРасчетовСКонтрагентом
		//СчетУчетаРасчетовПоАвансам
		//ПодразделениеКт
		РасшифровкаПлатежа.Добавить(НСтр);
	КонецЦикла;
	СтруктураВыгрузки.Вставить(РасшифровкаПлатежа);
...
//!!
Процедура ВыгрузитьПриобретение(СсылкаНаОбъект, ОбъектИзменен, ТаблицаВыгрузки, ОписаниеОшибок)
	...
	СтруктураВыгрузки.Вставить("СчетУчетаРасчетов",КодСчетаРегл(ПолучитьСчетДляОтраженияВРеглУчете(Перечисления.ВидыСчетовРеглУчета.РасчетыСПоставщиками,СсылкаНаОбъект.Организация,СсылкаНаОбъект.ГруппаФинансовогоУчета,СсылкаНаОбъект.Склад)));
	СтруктураВыгрузки.Вставить("СчетУчетаАвансов",КодСчетаРегл(ПолучитьСчетДляОтраженияВРеглУчете(Перечисления.ВидыСчетовРеглУчета.АвансыВыданные,СсылкаНаОбъект.Организация,СсылкаНаОбъект.ГруппаФинансовогоУчета,СсылкаНаОбъект.Склад)));
	...
	Для Каждого стрТовары Из СсылкаНаОбъект.Товары Цикл
		...
		СчетУчета = ПолучитьСчетУчетаИзНастроек(Перечисления.ВидыСчетовРеглУчета.НаСкладе,СсылкаНаОбъект.Организация,стрТовары.Номенклатура.ГруппаФинансовогоУчета,стрТовары.Склад);		
		структураСтрокаТовары.Вставить("СчетУчета",СформироватьУИДДляВыгрузки(СчетУчета));
		
		СчетУчетаНДС = ПланыСчетов.Хозрасчетный.НДСпоПриобретеннымМПЗ;
		структураСтрокаТовары.Вставить("СчетУчетаНДС",СформироватьУИДДляВыгрузки(СчетУчетаНДС));
...
//!!
Процедура ВыгрузитьРеализацию(СсылкаНаОбъект, ОбъектИзменен, ТаблицаВыгрузки, ОписаниеОшибок)
	...
	//	Если СсылкаНаОбъект.Склад.ЭтоГруппа Тогда
	...
	//	
	//КонецЕсли;	
	//БанковскийСчетКонтрагента ??? не нужен
	...
	Для Каждого Склад из Склады Цикл
		...
		Если НЕ НеДобавлятьСтруктуруПоСкладу Тогда
			...
			ВыгрузитьСклад(Склад, Ложь, ТаблицаВыгрузки, ОписаниеОшибок);
...
Процедура ВыгрузитьОстаткиПоСчету(СсылкаНаОбъект, ТаблицаВыгрузки, ОписаниеОшибок)
	...
	Для Каждого ОргПодр Из ТабОрганизацииПодразделения Цикл
		...
		ВыгрузитьОрганизацию(ОргПодр.Организация, Ложь, ТаблицаВыгрузки, ОписаниеОшибок);
		...
		Если ОргПодр.Подразделение <> Null тогда 
			...
			ВыгрузитьПодразделение(ОргПодр.Подразделение, ОргПодр.Организация, Ложь, ТаблицаВыгрузки, ОписаниеОшибок);
		Иначе
		...
		Для Каждого Стр Из СтрокиОстатков Цикл
				...
				Если Стр.Субконто1 <> Неопределено Тогда
					...
					ИначеЕсли СтрНайти(Нстр.ТипСубконто1, "DocumentRef") = 0 Тогда
						...
						Если ТипЗнч(Стр.Субконто1) = Тип("СправочникСсылка.Организации") Тогда     
							ВыгрузитьОрганизациюКакКонтрагента(Стр.Субконто1, Ложь, ТаблицаВыгрузки, ОписаниеОшибок);
						ИначеЕсли Стр.Счет.КодБыстрогоВыбора = "0803" Тогда
							ВыгрузитьОбъектСтроительства(Стр.Субконто1, Ложь, ТаблицаВыгрузки, ОписаниеОшибок);							
						Иначе	
							//ВыгрузитьОбъект(Стр.Субконто1, Ложь, ТаблицаВыгрузки, ОписаниеОшибок, МассивОбработанныхОбъектов); 
							ВыгрузитьОбъект(Стр.Субконто1, Ложь, ТаблицаВыгрузки, ОписаниеОшибок); 
						КонецЕсли;
					Иначе
						Нстр.Вставить("Субконто1", Строка(Стр.Субконто1.УникальныйИдентификатор()));							
					КонецЕсли;					
				...
				Если Стр.Субконто2 <> Неопределено Тогда
					...
					ИначеЕсли СтрНайти(Нстр.ТипСубконто2, "DocumentRef") = 0 Тогда
						...
						Если ТипЗнч(Стр.Субконто2) = Тип("СправочникСсылка.Организации") Тогда     
							ВыгрузитьОрганизациюКакКонтрагента(Стр.Субконто2, Ложь, ТаблицаВыгрузки, ОписаниеОшибок);
						Иначе	
							//ВыгрузитьОбъект(Стр.Субконто2, Ложь, ТаблицаВыгрузки, ОписаниеОшибок, МассивОбработанныхОбъектов);
							ВыгрузитьОбъект(Стр.Субконто2, Ложь, ТаблицаВыгрузки, ОписаниеОшибок);
						КонецЕсли;
					Иначе
						Нстр.Вставить("Субконто2", Строка(Стр.Субконто2.УникальныйИдентификатор()));						
					КонецЕсли;	
				...
				Если Стр.Субконто3 <> Неопределено Тогда
					...
					Если СтрНайти(Нстр.ТипСубконто3, "DocumentRef") = 0 Тогда
						Если ТипЗнч(Стр.Субконто3) = Тип("СправочникСсылка.Организации") Тогда     
							ВыгрузитьОрганизациюКакКонтрагента(Стр.Субконто3, Ложь, ТаблицаВыгрузки, ОписаниеОшибок);
						Иначе	
							//ВыгрузитьОбъект(Стр.Субконто3, Ложь, ТаблицаВыгрузки, ОписаниеОшибок, МассивОбработанныхОбъектов);
							ВыгрузитьОбъект(Стр.Субконто3, Ложь, ТаблицаВыгрузки, ОписаниеОшибок);
						КонецЕсли;	
					КонецЕсли;	
				Иначе     
			...
			ИначеЕсли СсылкаНаОбъект.КодБыстрогоВыбора = "70" Тогда
				...
				Если СтрокаДопДанных.Количество() > 0 Тогда
					...
					ВыгрузитьСотрудника(СтрокаДопДанных[0].Сотрудник, Ложь, ТаблицаВыгрузки, ОписаниеОшибок);
				КонецЕсли;
			...
			ИначеЕсли СсылкаНаОбъект.КодБыстрогоВыбора = "0101" Тогда
				...
				Если СтрокаДопДанных.Количество() > 0 Тогда
					...
					Для каждого ЗнСтруктуры из Нстр Цикл 
						...
						//ВыгрузитьОбъект(СсылкаНаЗначение, Ложь, ТаблицаВыгрузки, ОписаниеОшибок, МассивОбработанныхОбъектов);
						ВыгрузитьОбъект(СсылкаНаЗначение, Ложь, ТаблицаВыгрузки, ОписаниеОшибок);
					КонецЦикла;
				...
				Для Каждого СтрПартии из СтрокиПартий Цикл
					...
					Иначе     
						...
						Если ЗначениеЗаполнено(СтрПартии.Договор) Тогда
							...
							ВыгрузитьДоговор(СтрПартии.Договор, Ложь, ТаблицаВыгрузки, ОписаниеОшибок);
						Иначе
						...
						Если ЗначениеЗаполнено(СтрПартии.Контрагент) Тогда
							...
							Если ТипЗнч(СтрПартии.Контрагент) = Тип("СправочникСсылка.Организации") Тогда
								ВыгрузитьОрганизациюКакКонтрагента(СтрПартии.Контрагент, Ложь, ТаблицаВыгрузки, ОписаниеОшибок);
							Иначе     
								ВыгрузитьКонтрагента(СтрПартии.Контрагент, Ложь, ТаблицаВыгрузки, ОписаниеОшибок);
							КонецЕсли;
						Иначе
...
//Функция ВыгрузитьОбъект(СсылкаНаОбъект = Неопределено, ОбъектИзменен = Истина, ТаблицаВыгрузки = Неопределено, ОписаниеОшибок = Неопределено, МассивОбработанныхОбъектов)
Функция ВыгрузитьОбъект(СсылкаНаОбъект = Неопределено, ОбъектИзменен = Истина, ТаблицаВыгрузки = Неопределено, ОписаниеОшибок = Неопределено)
	...
	//Если МассивОбработанныхОбъектов.Найти(СсылкаНаОбъект) <> Неопределено Тогда  
	//	Возврат Истина;
	//КонецЕсли;
	...
	Если ТипОбъекта = Тип("СправочникСсылка.Валюты") Тогда
		ВыгрузитьВалюту(СсылкаНаОбъект, ОбъектИзменен, ТаблицаВыгрузки, ОписаниеОшибок);	
	ИначеЕсли ТипОбъекта = Тип("СправочникСсылка.Организации") Тогда                                                       
		ВыгрузитьОрганизацию(СсылкаНаОбъект, ОбъектИзменен, ТаблицаВыгрузки, ОписаниеОшибок);	
	ИначеЕсли ТипОбъекта = Тип("СправочникСсылка.СтруктураПредприятия") Тогда
		ВыгрузитьПодразделение(СсылкаНаОбъект, Справочники.Организации.ПустаяСсылка(), Истина, ТаблицаВыгрузки, ОписаниеОшибок);	
	ИначеЕсли ТипОбъекта = Тип("СправочникСсылка.Склады") Тогда
		ВыгрузитьСклад(СсылкаНаОбъект, ОбъектИзменен, ТаблицаВыгрузки, ОписаниеОшибок);	
	ИначеЕсли ТипОбъекта = Тип("СправочникСсылка.ФизическиеЛица") Тогда
		ВыгрузитьФизЛицо(СсылкаНаОбъект, ОбъектИзменен, ТаблицаВыгрузки, ОписаниеОшибок);	
	ИначеЕсли ТипОбъекта = Тип("СправочникСсылка.Пользователи") Тогда
		ВыгрузитьПользователя(СсылкаНаОбъект, ОбъектИзменен, ТаблицаВыгрузки, ОписаниеОшибок);	
	ИначеЕсли ТипОбъекта = Тип("СправочникСсылка.Контрагенты") Тогда
		ВыгрузитьКонтрагента(СсылкаНаОбъект, ОбъектИзменен, ТаблицаВыгрузки, ОписаниеОшибок);	
	ИначеЕсли ТипОбъекта = Тип("СправочникСсылка.ДоговорыКонтрагентов") 
		...
		ИЛИ ТипОбъекта = Тип("СправочникСсылка.ДоговорыАренды") Тогда
		ВыгрузитьДоговор(СсылкаНаОбъект, ОбъектИзменен, ТаблицаВыгрузки, ОписаниеОшибок);//!!	
	ИначеЕсли ТипОбъекта = Тип("СправочникСсылка.РегистрацииВНалоговомОргане") Тогда
		ВыгрузитьРегистрациюВНалоговомОргане(СсылкаНаОбъект, ОбъектИзменен, ТаблицаВыгрузки, ОписаниеОшибок);
	ИначеЕсли ТипОбъекта = Тип("СправочникСсылка.УчетныеЗаписиДокументооборота") Тогда
		ВыгрузитьУчетнуюЗаписьДокументооборота(СсылкаНаОбъект, ОбъектИзменен, ТаблицаВыгрузки, ОписаниеОшибок);
	ИначеЕсли ТипОбъекта = Тип("СправочникСсылка.НалоговыеОрганы") Тогда
		ВыгрузитьНалоговыйОрган(СсылкаНаОбъект, ОбъектИзменен, ТаблицаВыгрузки, ОписаниеОшибок);
	ИначеЕсли ТипОбъекта = Тип("СправочникСсылка.Номенклатура") Тогда
		ВыгрузитьНоменклатуру(СсылкаНаОбъект, ОбъектИзменен, ТаблицаВыгрузки, ОписаниеОшибок);	
	ИначеЕсли ТипОбъекта = Тип("СправочникСсылка.УпаковкиЕдиницыИзмерения") Тогда
		ВыгрузитьЕдиницуИзмерения(СсылкаНаОбъект, ОбъектИзменен, ТаблицаВыгрузки, ОписаниеОшибок);	
	ИначеЕсли ТипОбъекта = Тип("СправочникСсылка.БанковскиеСчетаКонтрагентов")
		...
		ВыгрузитьБанковскийСчет(СсылкаНаОбъект, ОбъектИзменен, ТаблицаВыгрузки, ОписаниеОшибок);
	ИначеЕсли ТипОбъекта = Тип("СправочникСсылка.КлассификаторБанков") Тогда
		ВыгрузитьБанк(СсылкаНаОбъект, ОбъектИзменен, ТаблицаВыгрузки, ОписаниеОшибок);
	ИначеЕсли ТипОбъекта = Тип("ПланВидовХарактеристикСсылка.СтатьиРасходов") Тогда
		ВыгрузитьСтатьюРасходов(СсылкаНаОбъект, ОбъектИзменен, ТаблицаВыгрузки, ОписаниеОшибок);
	ИначеЕсли ТипОбъекта = Тип("ПланВидовХарактеристикСсылка.СтатьиДоходов") Тогда
		ВыгрузитьСтатьюДоходов(СсылкаНаОбъект, ОбъектИзменен, ТаблицаВыгрузки, ОписаниеОшибок);
	ИначеЕсли ТипОбъекта = Тип("СправочникСсылка.Резервы") Тогда
		ВыгрузитьРезервыОценочныеОбязательства(СсылкаНаОбъект, ОбъектИзменен, ТаблицаВыгрузки, ОписаниеОшибок);
	ИначеЕсли ТипОбъекта = Тип("СправочникСсылка.ЦенныеБумаги") Тогда
		ВыгрузитьЦенныеБумаги(СсылкаНаОбъект, ОбъектИзменен, ТаблицаВыгрузки, ОписаниеОшибок);		
	ИначеЕсли ТипОбъекта = Тип("СправочникСсылка.ОбъектыЭксплуатации") Тогда
		ВыгрузитьОбъектЭксплуатации(СсылкаНаОбъект, ОбъектИзменен, ТаблицаВыгрузки, ОписаниеОшибок);
	ИначеЕсли ТипОбъекта = Тип("СправочникСсылка.НематериальныеАктивы") Тогда
		ВыгрузитьНематериальныйАктив(СсылкаНаОбъект, ОбъектИзменен, ТаблицаВыгрузки, ОписаниеОшибок);  		
	ИначеЕсли ТипОбъекта = Тип("СправочникСсылка.Сотрудники") Тогда
		ВыгрузитьСотрудника(СсылкаНаОбъект, ОбъектИзменен, ТаблицаВыгрузки, ОписаниеОшибок);
	ИначеЕсли ТипОбъекта = Тип("СправочникСсылка.СтатьиДвиженияДенежныхСредств") Тогда
		ВыгрузитьСтатьюДДС(СсылкаНаОбъект, ОбъектИзменен, ТаблицаВыгрузки, ОписаниеОшибок);
	ИначеЕсли ТипОбъекта = Тип("СправочникСсылка.СерверыДокументооборота") Тогда
		ВыгрузитьСерверДокументооборота(СсылкаНаОбъект, Ложь, ТаблицаВыгрузки, ОписаниеОшибок);
	ИначеЕсли ТипОбъекта = Тип("ДокументСсылка.АвансовыйОтчет") Тогда
		ВыгрузитьАвансовыйОтчет(СсылкаНаОбъект, ОбъектИзменен, ТаблицаВыгрузки, ОписаниеОшибок);
	ИначеЕсли ТипОбъекта = Тип("ДокументСсылка.ВзаимозачетЗадолженности") Тогда
		ВыгрузитьВзаимозачетЗадолженности(СсылкаНаОбъект, ОбъектИзменен, ТаблицаВыгрузки, ОписаниеОшибок);
	ИначеЕсли ТипОбъекта = Тип("ДокументСсылка.ВнутреннееПотреблениеТоваров") Тогда
		ВыгрузитьВнутреннееПотреблениеТоваров(СсылкаНаОбъект, ОбъектИзменен, ТаблицаВыгрузки, ОписаниеОшибок);				
	ИначеЕсли ТипОбъекта = Тип("ДокументСсылка.ВозвратТоваровОтКлиента") Тогда
		ВыгрузитьВозвратТоваровОтКлиента(СсылкаНаОбъект, ОбъектИзменен, ТаблицаВыгрузки, ОписаниеОшибок);
	ИначеЕсли ТипОбъекта = Тип("ДокументСсылка.ОприходованиеИзлишковТоваров") Тогда
		ВыгрузитьОприходованиеИзлишков(СсылкаНаОбъект, ОбъектИзменен, ТаблицаВыгрузки, ОписаниеОшибок);
	ИначеЕсли ТипОбъекта = Тип("ДокументСсылка.СписаниеНедостачТоваров") Тогда
		ВыгрузитьСписаниеНедостач(СсылкаНаОбъект, ОбъектИзменен, ТаблицаВыгрузки, ОписаниеОшибок);		
		
	ИначеЕсли ТипОбъекта = Тип("ДокументСсылка.ОперацияПоПлатежнойКарте") Тогда
		ВыгрузитьОперациюПоПлатежнойКарте(СсылкаНаОбъект, ОбъектИзменен, ТаблицаВыгрузки, ОписаниеОшибок);
	//Самойлов А.С. Начало 2022-09-23 #	
	ИначеЕсли ТипОбъекта = Тип("СправочникСсылка.СтраныМира") Тогда
		ВыгрузитьСтрану(СсылкаНаОбъект, ОбъектИзменен, ТаблицаВыгрузки, ОписаниеОшибок);
	ИначеЕсли ТипОбъекта = Тип("ДокументСсылка.ПриобретениеТоваровУслуг") Тогда
		ВыгрузитьПриобретение(СсылкаНаОбъект, ОбъектИзменен, ТаблицаВыгрузки, ОписаниеОшибок);
	//Самойлов А.С. Конец  2022-09-23		
	ИначеЕсли ТипОбъекта = Тип("ДокументСсылка.НачислениеДивидендов") Тогда
		ВыгрузитьНачислениеДивидендов(СсылкаНаОбъект, ОбъектИзменен, ТаблицаВыгрузки, ОписаниеОшибок);
	ИначеЕсли ТипОбъекта = Тип("ПланСчетовСсылка.Хозрасчетный") Тогда
		//ВыгрузитьОстаткиПоСчету(СсылкаНаОбъект, ТаблицаВыгрузки, ОписаниеОшибок, МассивОбработанныхОбъектов);
	...
	//МассивОбработанныхОбъектов.Добавить(СсылкаНаОбъект); //!!
...
Процедура ВыгрузитьДопОбъект(СтруктураОбъекта, ДопТаблицаВыгрузки, ОписаниеОшибок)
	...
	Если ТипОбъекта = Тип("СправочникСсылка.Организации") Тогда
		Если СтрНайти(СтруктураОбъекта.ТипПриемника, "Контрагент") > 0 Тогда
			ВыгрузитьОрганизациюКакКонтрагента(ОбъектСсылка, Истина, ДопТаблицаВыгрузки, ОписаниеОшибок);
		Иначе
			ВыгрузитьОрганизацию(ОбъектСсылка, Истина, ДопТаблицаВыгрузки, ОписаниеОшибок);
		КонецЕсли;
	ИначеЕсли ТипОбъекта = Тип("СправочникСсылка.СтруктураПредприятия")	Тогда
		ОрганизацияПодразделенияСтрока = Прав(СтруктураОбъекта.УИДОрганизации, 37);
		...
		Если НЕ ОрганизацияПодразделенияСсылка.Пустая() Тогда
			ВыгрузитьПодразделение(ОбъектСсылка, ОрганизацияПодразделенияСсылка, Истина, ДопТаблицаВыгрузки, ОписаниеОшибок);
		КонецЕсли;	
	Иначе     
		ВыгрузитьОбъект(ОбъектСсылка, Истина, ДопТаблицаВыгрузки, ОписаниеОшибок);	
	КонецЕсли;	
...
Процедура ОтправитьПакет(Узел, ТаблицаВыгрузки, НомерСообщения)
	...
	Пока РезультатВыгрузки.requestedObj.Количество() > 0 И текНомерПопытки <= МаксПопыток Цикл
		...
		Для ИндексОбъекта = 0 По РезультатВыгрузки.requestedObj.Количество() - 1 Цикл
			...
			Если текКоличествоСтрок <> ТаблицаВыгрузки.Количество() Тогда
				//Элемент добавлен в конец таблицы, надо пересунуть его на свое место
				ЗаполнитьЗначенияСвойств(ТаблицаВыгрузки.Вставить(ИндексОбъекта),ТаблицаВыгрузки[ТаблицаВыгрузки.Количество() - 1]);
	...
	Если РезультатВыгрузки.errorCode <> 0 Тогда   
		...
		"errorDescription: " + РезультатВыгрузки.errorDescription + Символы.ПС + 
		"errorObj" + РезультатВыгрузки.errorObj;
...
Процедура ВыполнитьВыгрузку() Экспорт
	...
	Пока ТаблицаДанных.Количество() > 0 Цикл
		...
		Для Каждого СтрокаДанные Из ТаблицаДанных Цикл
			...
			Попытка
				...
				Если НЕ ВыгрузитьОбъект(СтрокаДанные.Объект, Истина, ТаблицаВыгрузки, ОписаниеОшибок) Тогда 
					ПланыОбмена.УдалитьРегистрациюИзменений(Узел, СтрокаДанные.Объект);
					Продолжить;
				КонецЕсли;		 
	...
	//МассивОбъектовТекущейВыгрузки = Новый Массив;
	...
	//		//
	//		Если СтрНайти(СтрокаДанные.ТипОбъекта, "Документ") > 0 
	//			И текДата - СтрокаДанные.Объект.Дата > Узел.ВыгружатьЗаПоследниеДней * 3600 * 24 Тогда  
	//			
	//			ЗаписьЖурналаРегистрации("Выгрузка в бухгалтерию", УровеньЖурналаРегистрации.Предупреждение, , СтрокаДанные.Объект,
	//			"Объект не выгружен, так как попадает в закрытый период");
	//			ПланыОбмена.УдалитьРегистрациюИзменений(Узел, СтрокаДанные.Объект); 
	...
	//		КонецЕсли;
	//		
	//		//Если НЕ ВыгрузитьОбъект(СтрокаДанные.Объект, Истина, ТаблицаВыгрузки, ОписаниеОшибок, МассивОбработанныхОбъектов) Тогда 			
	//		Если НЕ ВыгрузитьОбъект(СтрокаДанные.Объект, Истина, ТаблицаВыгрузки, ОписаниеОшибок) Тогда 
	//			ПланыОбмена.УдалитьРегистрациюИзменений(Узел, СтрокаДанные.Объект);
	//			Продолжить;
	//		КонецЕсли;
	//		//МассивОбъектовТекущейВыгрузки.Добавить(СтрокаДанные.Объект);
	//		
	//		//Если ТаблицаВыгрузки.Количество() >= Узел.РазмерПорцииДанных Тогда
	//		//	ОтправитьПакет(Узел, ТаблицаВыгрузки, МассивОбъектовТекущейВыгрузки); 
	//		//	МассивОбъектовТекущейВыгрузки.Очистить();
	//		//КонецЕсли;  
	//		
	//	Исключение
	//		ЗаписьЖурналаРегистрации("Выгрузка в бухгалтерию", УровеньЖурналаРегистрации.Ошибка, , СтрокаДанные.Объект, ОписаниеОшибки());
	...
	//КонецЕсли;	
	//
	//ДопТаблицаВыгрузки = ТаблицаВыгрузки.СкопироватьКолонки();
	//
	//Попытка	
	//	
	//	сч = 0;
	//	мсвВыгрузки = Новый Массив;
	//	
	//	Для Каждого Стр Из ТаблицаВыгрузки Цикл
	//		
	//		Если сч = Узел.РазмерПорцииДанных Тогда
	//			ОтправитьПакет(Узел, мсвВыгрузки, ДопТаблицаВыгрузки);
	//			сч = 0;	
	//			мсвВыгрузки = Новый Массив;
	//		КонецЕсли;	
	//		
	//		мсвВыгрузки.Добавить(Стр.СтруктураВыгрузки);
	//		
	//		сч = сч + 1;
	//		
	//	КонецЦикла;	
	//	
	//	Если мсвВыгрузки.Количество() Тогда
	//		ОтправитьПакет(Узел, мсвВыгрузки, ДопТаблицаВыгрузки);
	//	КонецЕсли;	
	//	
	//	//Если ТаблицаВыгрузки.Количество() > 0 Тогда
